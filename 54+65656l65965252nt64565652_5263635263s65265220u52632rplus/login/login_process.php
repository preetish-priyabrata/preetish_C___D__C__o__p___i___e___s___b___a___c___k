...
//Array ( [email] => admin@ilab.com [password] => 1234 )
$userid=mysqli_real_escape_string($conn,trim($_POST['email']));
$password=mysqli_real_escape_string($conn,trim($_POST['password']));

if(!empty($password) && !empty($userid)){
 	$query_login="SELECT * FROM `ilab_master_admin` where `admin_email`='$userid' and `status`='1'";
    ...
    if($login_num_row==1){
    	...
    	if($password_db==$enter_password_hash){
    		$_SESSION['email_id']=trim($_POST['email']);
