							    ...
							    <div class="col-lg-8">
							    	<select name="location" required>
							    		...
							    		$query="SELECT DISTINCT `t_unique_id_machine` FROM `lt_master_machine__transfer_information`  ";
											...
											?>
												<option value="<?=$res['t_unique_id_machine']?>"  <?php if($res['t_unique_id_machine']==$location){echo "selected";}?>><?=$res['t_unique_id_machine']?></option>
												<?php
											}
						  ...
						  <label class="control-label col-lg-4 text-right"> Component Category * </label>
							 <div class="col-lg-8">
						       <select class="form-control" name="category_name" autocomplete="off"  id="demo" type="dropdown" required="">
	                    			...
	                    			<option value="<?php echo $rec_category['slno'];?>" <?php if($rec_category['slno']==$category_name){echo "selected";}?>><?=$rec_category['category_name'];?></option>
	             					 <?php }?>
					   	...
					   	<div class="table-responsive">
			        		<table id="example77" class="display nowrap" width="100%" cellspacing="0">
		        				...
		        				while ($res=mysqli_fetch_assoc($sql_exe)) {
		        					...
		        					<tr>
		        						<td><?=$x?></td>
									    <td><?=$res['fmgd_machine_no']?></td>
			 						    <td><?=$res['date']?></td>
			 						    <td><?=$res['fmgd_primary_id']?></td>
			 						    <td><?=$res['fmgd_category_name']?></td>
			 						    <?php $field_code=$res['fmgd_field_id'];
	        							...
	        							?>
	        							<td><?php echo "<p>".$result2['field_name']."</p>"?></td>
	        							<!-- <td><?=$res['fmgd_available_id']?></td> -->
			 						    <td><?=$res['total_issued']?></td>
