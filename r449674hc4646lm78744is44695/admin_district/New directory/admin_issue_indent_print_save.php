 ...
 // Array ( [date] => 26-03-2017 [time] => 05:46:22 pm [challen_no] => chal_478513 [indent_id] => ind001 [receiver_id] => ATHM [issuer_id] => Pat ) 
	...
	$issuer_id=$_REQUEST['issuer_id'];
 	$challen_no=$_REQUEST['challen_no'];
 	 ...
 	 $query_update_indent="UPDATE `rhc_master_indent` SET `status`='2' WHERE `indent_id`='$indent_id' ";
		$sql_exe_update_indent=mysqli_query($conn,$query_update_indent);

	 $query_update_status_challan="UPDATE `rhc_master_challan_no` SET `status`='0' WHERE `challen_no`='$challen_no'";
	...
	$date=date('Y-m-d');
	$time=date('H:i:s');
	$get_item="SELECT * FROM `rhc_master_itemdetails_challan_no` WHERE `challan_no`='$challen_no'";
