...
if($_POST['place_status']!="" && $_POST['place_id']!="" && $_POST['itemtypes']!="" && $_POST['itemcodes']!="" && $_POST['ids']!=""){
	...
	if($place_status==2){
		...
		// tota; indent value
		 ...
		 $query_table_received="SELECT SUM(`quantity_received`) FROM `rhc_master_district_item_challen_no` WHERE `item_code`='$item_code' and `item_type`='$item_type' and `status`='1' And `receiver_place_id`='$place_id' And `date_creation` Between '$starting_date' AND '$ending_date'";
