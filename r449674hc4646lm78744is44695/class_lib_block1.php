	    ...
	    // here it is check by count where separates by comma
	     ...
	     if($simalar=="0"){
	     	for ($i=0; $i <$count_details ; $i++) { 
		     	...
		     	if($item_quantitys==0){      
         			$query_update="UPDATE `rhc_master_stock_block_items` SET `item_quantity`='$item_quantitys',`date_creation`='$date',`time_creation`='$time',`status`='2' WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `distrct_place_id`='$raise_place_id' ";
     				...
     				$array_list[]=$item_code[$i]." ".$item_quantity[$i]." ".$item_type[$i];
      				...
      				if($item_quantitys>0){      
       					$query_update="UPDATE `rhc_master_stock_block_items` SET `item_quantity`='$item_quantitys',`date_creation`='$date',`time_creation`='$time',`status`='1' WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `distrct_place_id`='$raise_place_id'";
       					$updated_stock[] = array('item_codes' =>$item_codes ,'item_types' =>$item_types,'item_quantity' =>$item_quantitys );
       					$array_list[]=$item_code[$i]." ".$item_quantity[$i]." ".$item_type[$i];
