            ...
            <?php
              $x=1;
               ...
               $query_item="SELECT * FROM `rhc_master_stock_state_items` WHERE `state_place_id`='$state_id'";
                ...
                $query_issued="SELECT SUM(quantity_issued),SUM(quantity_received) FROM `rhc_master_district_item_details_challan_no` WHERE `receiver_place_id`='$district_id' and `item_code`='$item_code' and `item_type`='$item_types' and YEAR(`date_creation`) BETWEEN '$date_one' AND '$date_two'";

                $query_issued_transit="SELECT SUM(quantity_issued) FROM `rhc_master_district_item_details_challan_no` WHERE `receiver_place_id`='$district_id' and `status`!='1' and `item_code`='$item_code' and `item_type`='$item_types' and YEAR(`date_creation`) BETWEEN '$date_one' AND '$date_two'";
      ...
      <h3 class="text-center">Indent Raised From <?=$place_name?></h3>
        <table id="example1" class="table table-bordered table-striped table-hover text-center">
                ...
                $res_lis_type=mysqli_fetch_assoc($sql_exe_item_type);
                 ...
                 $item_names=$res_list['item_name'].'-'.$res_lis_type['item_type_name'];
                  $query_list="SELECT  * FROM rhc_master_dh_block_indent_id_details INNER JOIN rhc_master_dh_block_indent ON rhc_master_dh_block_indent_id_details.indent_id=rhc_master_dh_block_indent.indent_id WHERE rhc_master_dh_block_indent.indent_place_raised_by='$block_dh_uphc' and rhc_master_dh_block_indent_id_details.item_code='$item_code' and rhc_master_dh_block_indent_id_details.Item_tdate_twoype='$item_types' and  YEAR(rhc_master_dh_block_indent.date_creation) BETWEEN '$date_one' AND '$date_two'";
                     $sql_exe_list=mysqli_query($conn,$query_list);
                      $fetch=mysqli_fetch_array($sql_exe_list);

                    $query_issued="SELECT SUM(quantity_issued),SUM(quantity_received) FROM `rhc_master_dh_block_item_details_challan_no` WHERE `receiver_place_id`='$block_dh_uphc' and `item_code`='$item_code' and `item_type`='$item_type' and YEAR(`date_creation`) BETWEEN '$date_one' AND '$date_two' ";

                    $query_issued_transit="SELECT SUM(quantity_issued) FROM `rhc_master_dh_block_item_details_challan_no` WHERE `receiver_place_id`='$block_dh_uphc' and `status`!='1' and `item_code`='$item_code' and `item_type`='$item_type' and YEAR(`date_creation`) BETWEEN '$date_one' AND '$date_two' ";
           ...
           <table id="example1" class="table table-bordered table-striped table-hover text-center">
                ...
                $res_lis_type=mysqli_fetch_assoc($sql_exe_item_type);
                 ...
                 $item_names=$res_list['item_name'].'-'.$res_lis_type['item_type_name'];
                  ...
                  $query_list="SELECT * FROM rhc_master_phc_aphc_indent_id_details INNER JOIN rhc_master_phc_aphc_indent ON rhc_master_phc_aphc_indent_id_details.indent_id=rhc_master_phc_aphc_indent.indent_id WHERE rhc_master_phc_aphc_indent.indent_place_raised_by='$phc_aphc' and rhc_master_phc_aphc_indent_id_details.item_code='$item_code' and rhc_master_phc_aphc_indent_id_details.Item_type='$item_type'and  YEAR(rhc_master_phc_aphc_indent.date_creation) BETWEEN '$date_one' AND '$date_two' ";
                     $sql_exe_list=mysqli_query($conn,$query_list);
                     $fetch=mysqli_fetch_array($sql_exe_list);

                    $query_issued="SELECT SUM(quantity_issued),SUM(quantity_received) FROM `rhc_master_phc_aphc_item_details_challan_no` WHERE `receiver_place_id`='$block_dh_uphc' and `item_code`='$item_code' and `item_type`='$item_type' and YEAR(`date_creation`) BETWEEN '$year_one' AND '$date_two'";

                    $query_issued_transit="SELECT SUM(quantity_issued) FROM `rhc_master_phc_aphc_item_details_challan_no` WHERE `receiver_place_id`='$block_dh_uphc' and `status`!='1' and `item_code`='$item_code' and `item_type`='$item_type' and YEAR(`date_creation`) BETWEEN '$date_one' AND '$date_two'";
