...
//////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function phc_sub($mobiles,$message1s){
    	   ...
    	   // 		return phc_sub_indent($message1,$mobs);

    	   // }else{
    	   // 		if (preg_match('/update/',$message1)){
    	   // 			return phc_sub_update($message1,$mobs);
    	   // 		}else{
    	   // 			// echo "hello";
    	   // 			return phc_sub_iss_rec($message1,$mobs);
    	   // 		}
    	   		
    	   // }

    	  	if(preg_match('/cancel indent/',$message1)){ // cancelation of indent
    	    	return  phc_sub_cancel_indent($message1,$mobs);
    	   	}else{
    	   		if (preg_match('/update/',$message1)){
    	   			return  phc_sub_update($message1,$mobs);
    	   		}else{
    	   			if (preg_match('/cancel issue/',$message1)){ // cancelation of issue
    	   				...
    	   				return  phc_sub_cancel_issue($message1,$mobs);
    	   			}else{
...
//////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function phc_sub_indent($message_ind,$mobs1){
    	global $conn;
    	// include 'config.php';
    	$message_ind;
    	$mobs1;
	    ...
	    // here it is check by count where separates by comma
	     ...
	     if($simalar=="0"){
  			...
  			$time= date('H:i:s');
  			$da=date('md');
   			$query_intent_details="INSERT INTO `rhc_master_phc_asha_indent`(`slno`, `indent_place_raised_to`, `indent_place_raised_by`, `date_creation`, `time_creation`,`inserted_by_id`) VALUES (NULL,'$top_place_id','$raise_place_id' ,'$date','$time','$user')";
			   ...
			   //  		$array_list_user_mobile[]=$res_top_level_mobile['user_mobile'];
			    ...
			    // print_r($res);
			    	...
			    	$mobileno_top=implode(";",$array_list_user_mobile);
			  		$raise_level_mobile="SELECT * FROM `rhc_master_login_user` WHERE `sub_center_id`='$raise_place_id' and `user_designation`='7' And `status`='1'";
...
//////////////////////////////////////////////////////////////////////////////////////////////////
   	...
   	function phc_sub_update($message_ind,$mobs1){
    	global $conn;
	    ...
	    // here it is check by count where separates by comma
	     ...
	     if($simalar=="0"){
	     	for ($i=0; $i <$count_details ; $i++) { 
		     	...
		     	if($item_quantitys==0){      
         			$query_update="UPDATE `rhc_master_stock_phc_subcenter_items` SET `item_quantity`='$item_quantitys',`date_creation`='$date',`time_creation`='$time',`status`='2' WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `phc_sub_place_id`='$raise_place_id' ";
     				...
     				$array_list[]=$item_code[$i]." ".$item_quantity[$i]." ".$item_type[$i];
      				...
      				if($item_quantitys>0){      
       					$query_update="UPDATE `rhc_master_stock_phc_subcenter_items` SET `item_quantity`='$item_quantitys',`date_creation`='$date',`time_creation`='$time',`status`='1' WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `phc_sub_place_id`='$raise_place_id'";
       					$updated_stock[] = array('item_codes' =>$item_codes ,'item_types' =>$item_types,'item_quantity' =>$item_quantitys );
       					$array_list[]=$item_code[$i]." ".$item_quantity[$i]." ".$item_type[$i];
	     	...
	     	$query_get_stock="SELECT * FROM `rhc_master_stock_phc_subcenter_items` WHERE `phc_sub_place_id`='$raise_place_id'";
		    ...
		    $insert_query="INSERT INTO `rhc_master_update_phc_subcenter_stock`(`slno`, `updated_place_to`, `updated_place_from`, `present_info`, `updated_info`, `place_status`, `date`, `time`, `updated_user_id`) VALUES (NULL,'$top_place_id','$raise_place_id','$pres_stock','$updtd_stock','1','$date','$time','$user')";
...
function phc_sub_iss_rec($message1,$mobs){
	...
	$check_challan="SELECT * FROM `rhc_master_phc_asha_challan_no` WHERE `challen_no`='$values' and `receiver_place_id`='$sub_center_id'  and  `status`='0' or `status`='2' ";
	$sql_exe_check_challan=mysqli_query($conn,$check_challan);
	 $num_rows=mysqli_num_rows($sql_exe_check_challan);

	$check_indent1="SELECT * FROM `rhc_master_asha_indent` WHERE `indent_id`='$values' and `indent_place_raised_to`='$sub_center_id' and `place_status`='1' and  `status`='1'  ";
	...
	if(($num_rows==1) && ($num_rows1==0)){
		...
		return phc_sub_receive($message_value,$mobs_value,$challan_value,$item_details1,$receiver_id,$issuer_id);
		...
		$indent_value=$values;
		return phc_sub_issue($message_value,$mobs_value,$indent_value,$item_details1,$receiver_id,$issuer_id);
	}else{
...
//////////////////////////////////////////////////////////////////////////////////////////////////
// receiver section
function phc_sub_receive($message_value,$mobs_value,$challan_value,$item_details1,$receiver_id,$issuer_id){
	global $conn;
	    ...
	    if($cat!=0){
	    	...
	    	for ($j=0; $j <count($item_code); $j++) { 
	    		...
	    		$array_list[]=$item_codes." ".$item_quantity_issues." ".$item_types;
		    	 ...
		    	 $query_update_item_challan="UPDATE `rhc_master_phc_asha_item_details_challan_no` SET `quantity_received`='$item_quantity_issues',`status`='1' WHERE `challan_no`='$challan_no' and `item_code`='$item_codes' And `item_type`='$item_types'";
					...
					$query_get_item_code_id="SELECT * FROM `rhc_master_stock_phc_subcenter_items` WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `phc_sub_place_id`='$receiver_id'";
				...
				$item_quantityss=$result_detail['item_quantity'];
				 ...
				 if($cal==0){	
					$update="UPDATE `rhc_master_stock_phc_subcenter_items` SET `item_quantity`='$cal',`status`='1' WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `phc_sub_place_id`='$receiver_id'";
				}else{
					$update="UPDATE `rhc_master_stock_phc_subcenter_items` SET `item_quantity`='$cal',`status`='1' WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `phc_sub_place_id`='$receiver_id'";
				}
		    ...
		    for ($i=0; $i <count($batch_id_item_details) ; $i++) { 
			    ...
			    while ($res_batch_detail=mysqli_fetch_assoc($sql_exe_batch_details)) {
		    		...
		    		$$update_received_batches_info="UPDATE `rhc_master_phc_asha_receive_batch_detail_item` SET `rec_batch_no`='$batch_no',`rec_batch_expire`='$date_expire',`rec_batch_quantity`='$batch_quantity' WHERE`slno`='$REC_BATCH'";
					$sql_exe_received_batches_info=mysqli_query($conn,$update_received_batches_info);

    				 $query_get_item_code_id1="SELECT * FROM `rhc_master_stock_phc_subcenter_items` WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `phc_sub_place_id`='$receiver_id'";
					...
					$phc_sub_stock_batch_id=$result_detail1['phc_sub_stock_batch_id'];
						if($batch_no!=0){
							$batch_insert="INSERT INTO `rhc_master_stock_phc_batch_details`(`slno`, `phc_sub_stock_batch_id`, `batch_nos`, `date_exp`, `total_quantity`, `remaining_quantity`, `status`, `date_creation`, `time_creation`, `phc_sub_place_id`) VALUES (NULL,'$phc_sub_stock_batch_id','$batch_no','$date_expire','$batch_quantity','$batch_quantity','1','$date','$time','$receiver_id')";
...
// 																								//
//////////////////////////////////////////////////////////////////////////////////////////////////
function phc_sub_issue($message_value,$mobs_value,$indent_value,$item_details1,$receiver_id,$issuer_id){
// starting of dist_issue {
	...
	$indent_id=$indent_no=$arr[0]; // defined indent id
		    	...
		    	if($num_rows_get_details==0){
		    		...
		    		if($num_rows_get_details1==0){ // checking valid indent id for for state or not..
						...
						for ($i=0; $i <count($item_code) ; $i++) { 
							...
							$last_id = mysqli_insert_id($conn);
							$item_batch_id="ashablbatch00".$last_id;
						...
						// 	$qnt_issue_single=$qnt_issue[$i];
							...
							$query_check="SELECT * FROM `rhc_master_stock_phc_subcenter_items` WHERE `status`='1' AND `phc_sub_place_id`='$issuer_id' And `item_codes`='$item_code_single' And `item_types`='$Item_type_single'";
							...
							if($num_rows!=0){
								...
								$available_stock=$result_fetch['item_quantity'];
							 	$phc_sub_stock_batch_id=$result_fetch['phc_sub_stock_batch_id'];
							 	$remain_stock=$available_stock-$qnt_issue_single;
								if(0>$remain_stock){
									$issue_stock=$qnt_issue_single+($remain_stock);
									$qnt_issue1=$issue_stock;

									$query_batch="SELECT * FROM `rhc_master_stock_phc_subcenter_batch_details` WHERE `phc_sub_stock_batch_id`='$phc_sub_stock_batch_id' and `status`='1' and `date_exp`>'$date' ORDER BY YEAR(`date_exp`) ASC, MONTH(`date_exp`) ASC, DAY(`date_exp`) ASC";
									...
									if($num_rows1!=0){
										while ($res_batch=mysqli_fetch_assoc($sql_exe_batch)) {
											...
											if($issue_stocks<$qnt_issue1){
												if(0>$check_avaialble){
													...
													$item_details[]=array("phc_sub_stock_batch_id"=>"$phc_sub_stock_batch_id","item_code_single"=>"$item_code_single","Item_type_single"=>"$Item_type_single","batch_nos"=>"$batch_nos","remaining_quantity"=>"$remaining_quantity","date_exp"=>"$date_exp","remaining_quantity1"=>"$remaining_quantity1",'place_item_quantity'=>"$place_item_quantity",'slno'=>"$slnos");
										...
										$item_details[]=array("phc_sub_stock_batch_id"=>"0","item_code_single"=>"$item_code_single","Item_type_single"=>"$Item_type_single","batch_nos"=>"0","remaining_quantity"=>"0","date_exp"=>"0",'remaining_quantity1'=>"0",'place_item_quantity'=>"$place_item_quantity",'slno'=>"0");
								 	}
								}else{
								 	$query_batch="SELECT * FROM `rhc_master_stock_phc_subcenter_batch_details` WHERE `phc_sub_stock_batch_id`='$phc_sub_stock_batch_id' and `status`='1' and `date_exp`>'$date' ORDER BY YEAR(`date_exp`) ASC, MONTH(`date_exp`) ASC, DAY(`date_exp`) ASC";
									$sql_exe_batch=mysqli_query($conn,$query_batch);
					...
					//   exit;
							...
							for ($i=0; $i <count($item_batch_array) ; $i++) { 
								for ($j=0; $j <count($item_details) ; $j++) { 
									...
									if($ty==$ty1){
										...
										$phc_sub_stock_batch_ids=$item_details[$j]['phc_sub_stock_batch_id'];
										 	...
										 	$sql_exe_batch_insert=mysqli_query($conn,$query_store_batch);
												...
												if($sql_exe_batch_insert){
													...
													if($remain_qnt==0){
														...
														$stock_update_district="UPDATE `rhc_master_stock_phc_subcenter_batch_details` SET `remaining_quantity`='$remain_qnt',`status`='$status',`date_creation`='$date1',`time_creation`='$time1' WHERE `phc_sub_stock_batch_id`='$phc_sub_stock_batch_ids' AND `batch_nos`='$batch_no' and `slno`='$slno'";
													}else{
													 	$status="1";
														$stock_update_district="UPDATE `rhc_master_stock_phc_subcenter_batch_details` SET `remaining_quantity`='$remain_qnt',`status`='$status',`date_creation`='$date1',`time_creation`='$time1' WHERE `phc_sub_stock_batch_id`='$phc_sub_stock_batch_ids' AND `batch_nos`='$batch_no' and `slno`='$slno'";
													}
								...
								if($place_item_quantity!="0"){
									...
									if($stock_remain_qnt==0){
										$status1="2";
										$query_stock_item_update="UPDATE `rhc_master_stock_phc_subcenter_items` SET `item_quantity`='$stock_remain_qnt',`date_creation`='$date1',`time_creation`='$time1',`status`='$status1' WHERE `phc_sub_stock_batch_id`='$phc_sub_stock_batch_ids' and `item_codes`='$code_item' and `item_types`='$type_item' and `phc_sub_place_id`='$issuer_id'  ";
							...
							$code_details= implode(",",$array_list);
							$top_level_mobile="SELECT * FROM `rhc_master_login_user` WHERE `sub_center_id`='$issuer_id' and `user_designation`='7' and `status`='1' ";
							$sql_exe_top_level_mobile=mysqli_query($conn,$top_level_mobile);
							while($res_top_level_mobile=mysqli_fetch_assoc($sql_exe_top_level_mobile)){
							// print_r($res);
								$array_list_user_mobile[]=$res_top_level_mobile['user_mobile'];
	 ...
	 // print_r($res);
								...
								$raise_level_mobile4="SELECT * FROM `rhc_master_login_user` WHERE `place_phc_aphc_id`='$place_phc_aphc_id' and `user_designation`='5' and `status`='1' ";
