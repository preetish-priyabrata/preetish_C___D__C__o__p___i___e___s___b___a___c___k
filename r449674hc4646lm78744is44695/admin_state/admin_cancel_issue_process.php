...
$query_challan="SELECT * FROM `rhc_master_district_challan_no` WHERE `challen_no`='$challen'";
...
$result_fetch_challen =mysqli_fetch_assoc($sql_exe_challan); 
 ...
 $issuer_id=$place_id=$result_fetch_challen['issuer_place_id'];
   ...
   $query_list="SELECT * FROM `rhc_master_district_indent` WHERE `indent_place_raised_by`='$place_id1' and `indent_id`='$indent_id' and `status`='1'";
  ...
  $x=1;
  $query_list_item="SELECT * FROM `rhc_master_district_item_details_challan_no` WHERE `challan_no`='$challen'";
  ...
  while ($res_item=mysqli_fetch_assoc($sql_exe_list_item)) {
    ...
    $query_batch_details="SELECT * FROM `rhc_master_district_receive_batch_detail_item` WHERE `item_batch_id`='$item_batch_idm'";
 ...
 // print_r($batch_no);
  ...
  for ($i=0; $i <count($item_code) ; $i++) { 
    ...
    $sql_stock_item="SELECT * FROM `rhc_master_stock_state_items` WHERE `item_codes`='$item_codes' And `item_types`='$Item_types' and `state_place_id`='$issuer_id'";//item_quantity
    ...
    if($num_stock_item!=0){
      ...
      if($qty_item==0){
        $update_stock="UPDATE `rhc_master_stock_state_items` SET `item_quantity`='0' ,`status`='2' WHERE `slno`='$slno'";
      }else{
        $update_stock="UPDATE `rhc_master_stock_state_items` SET `item_quantity`='$qty_item' ,`status`='1' WHERE `slno`='$slno'";
      }
      $sql_exe=mysqli_query($conn,$update_stock);
    }
    // detele in issue stock table
    $sql_detele="DELETE FROM `rhc_master_district_item_details_challan_no` WHERE `item_batch_id`='$item_batch_ids' AND `challan_no`='$challen'";
  ...
  for ($i=0; $i <count($item_code) ; $i++) { 
    for ($j=0; $j <count($item_code_batch) ; $j++) { 
      ...
      if($ty==$ty1){
        ...
        // stock update
        $sql_stock_item="SELECT * FROM `rhc_master_stock_state_items` WHERE `item_codes`='$item_code_batchs' And `item_types`='$Item_type_batchs' and `state_place_id`='$issuer_id'";//item_quantity
        $sql_exe_stock_item=mysqli_query($conn,$sql_stock_item);
        $num_stock_item=mysqli_num_rows($sql_exe_stock_item);
        if($num_stock_item!=0){
          $stock_fetch_item=mysqli_fetch_assoc($sql_exe_stock_item);
          $state_stock_batch_id=$stock_fetch_item['state_stock_batch_id'];
          $stock_batch="SELECT * FROM `rhc_master_stock_state_batch_details` WHERE `state_stock_batch_id`='$state_stock_batch_id' and `batch_nos`='$batch_nos'";
          ...
          if($num_stock_item_batch==1){
            ...
            if($qty_remaining==0){
              $update_stock_batch="UPDATE `rhc_master_stock_state_batch_details` SET `status`='2',`remaining_quantity`='0' WHERE `slno`='$slno'";
            }else{
              $update_stock_batch="UPDATE `rhc_master_stock_state_batch_details` SET `status`='1',`remaining_quantity`='$qty_remaining' WHERE `slno`='$slno'";
            }
        ...
        // 
        $del_batch_issue_query="DELETE FROM `rhc_master_district_receive_batch_detail_item` WHERE `item_batch_id`='$item_batch_idss' and `item_code`='$item_code_batchs' and `batch_no`='$batch_nos'";
  ...
  $det_challan="DELETE FROM `rhc_master_district_challan_no` WHERE `indent_id`='$indent_id' and `challen_no`='$challen'";
