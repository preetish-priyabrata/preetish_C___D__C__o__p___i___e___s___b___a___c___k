...
// print_r($_REQUEST);
  ...
  $date=date('Y-m-d');
  $time=date('H:i:s');
  $get_indent="SELECT * FROM `rhc_master_indent_id_details` WHERE `indent_id`='$indent_id'";
  ...
  $top_level_mobile="SELECT * FROM `rhc_master_login_user` WHERE `place_state_id`='$top_place_id' and `user_designation`='1'";
  ...
  while($res_top_level_mobile=mysqli_fetch_assoc($sql_exe_top_level_mobile)){
   ...
   $raise_level_mobile="SELECT * FROM `rhc_master_login_user` WHERE `place_district_id`='$raise_place_id' and `user_designation`='2'";
