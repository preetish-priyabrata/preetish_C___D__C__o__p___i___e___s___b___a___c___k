  ...
  <div class="content-wrapper">
    ...
    <div class="box box-info">
      ...
      <div class="panel panel-default">
        <div class="panel-heading text-center" style="background-color: lightpink;"><strong>Qc Stock Issue Return Detail</strong></div>
        <div class="demo">
          <div class="panel-body">
          <form action="admin_stock_entry_issue1.php" method="POST">
                <?php 
              ...
              <table class="table " border="2">
                <tr>
                 <th>Item Code</th>
                  <th>Item Type</th>
                  <th>Quantity</th>
                  <?php if($res_status['status']==1 && $res_status['status']!=0){?>
                  <th>Issued Qty</th>
                  <?php if($res_status['status']==2){?>
                  <th>Return Stock</th>
                  <?php }?>
               ...
               <?php 
                 ...
                 <td><?=$item_code=$res['item_code']?></td>
                  ...
                  <input type="hidden" name="item_code1[]" value="<?=$item_code?>" required="">
                     <input type="hidden" name="item_type1[]" value="<?=$item_type?>" required="">
                       ...
                       <input type="hidden" name="issue_item[]" value="<?=$issue_item?>" required="">
                 ...
                 <td><table class="table" border="1">
                   ...
                   <input type="hidden" name="item_code[]" value="<?=$item_code?>" required="">
                    ...
                    <td>
                       <?php if($res_status['status']==1){?>
                    
                    <?=$issued=$res_batch['test_issue_quantity']?>
                     <input type="hidden" name="batch_test_issue_quantity[]" value="<?=$res_batch['test_issue_quantity']?>" required="">
                    <?php }?>
                    </td>
                     <?php if($res_status['status']==1 || $res_status['status']==2){?>
                     <td>
            ...
            <?php if($res_status['status']!=0 &&  $res_status['status']==1 && $res_status['status']!=2){?>
