...
include 'config.php';
 $msg = new \Preetish\FlashMessages\FlashMessages();
  ...
  $query_list="SELECT * FROM `rhc_master_phc_aphc_indent` WHERE `indent_place_raised_to`='$place_id' and `indent_id`='$indent_id' AND `status`='1'";
  ...
  if(qnt_issue!=""){
    if(parseInt(qnt_issue)>parseInt(item_quantity)){
      ...
      document.getElementById("error"+id).innerHTML = "Please Insert a value less  Than or equalto ("+item_quantity+")";
       ...
       $.ajax({
                type:'POST',
                url:'admin_get_item_batch_details_block.php',
  ...
  if(qnt_issues!=""){
    if(parseInt(item_quantitys)>=parseInt(qnt_issues)){
      document.getElementById("error"+id).innerHTML = "";
       $.ajax({
                type:'POST',
                url:'admin_get_item_batch_details_blocks.php',
