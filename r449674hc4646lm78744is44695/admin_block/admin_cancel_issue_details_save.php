	...
	for ($i=0; $i <count($item_code) ; $i++) { 
		for ($j=0; $j <count($item_code_batch) ; $j++) { 
			...
			if($ty==$ty1){
				...
				if($num_stock_item!=0){
					$stock_fetch_item=mysqli_fetch_assoc($sql_exe_stock_item);
					$block_stock_batch_id=$stock_fetch_item['block_stock_batch_id'];
					$stock_batch="SELECT * FROM `rhc_master_stock_block_batch_details` WHERE `block_stock_batch_id`='$block_stock_batch_id' and `batch_nos`='$batch_nos'";
