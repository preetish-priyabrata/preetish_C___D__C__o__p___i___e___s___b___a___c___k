...
if($_SESSION['admin_emails']){
	...
	require 'config.php';
 	...
 	// print_r($_REQUEST);
 	// exit;
 	$block_place_id=$_SESSION['place_id'];
...
$batch_no=$_REQUEST['batch_no'];
	//here we have get information of batch total stock and avaliable stock in particular 
	 $sql_query="SELECT * FROM `rhc_master_stock_block_batch_details` WHERE `slno`='$slno' and `batch_nos`='$batch_no'and `block_stock_batch_id`='$batch_store_id'";
	...
	if($num!=0){
		...
		$state_item="SELECT * FROM `rhc_master_stock_block_items` WHERE `block_stock_batch_id`='$batch_store_id' and`block_place_id`='$block_place_id'";
