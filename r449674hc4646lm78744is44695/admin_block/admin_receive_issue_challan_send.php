...
for ($i=0; $i <count($item_code) ; $i++) { 
  ...
  $quantity_receivedss=$item_quantity_issue[$i];
    ...
    if($x_item_batch==$x_item_rec){
      if($qnt!=0){
        ...
        $sql_exe_received_batches_info=mysqli_query($conn,$update_received_batches_info);
          ...
          $block_check="SELECT * FROM `rhc_master_stock_block_batch_details` WHERE `block_stock_batch_id`='$block_stock_batch_id' and `batch_nos`='$batch_nos'";
