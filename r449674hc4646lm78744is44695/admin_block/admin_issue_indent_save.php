...
$date1=date('Y-m-d');
$time1=date('H:i:s');
// Array ( [date] => 2017-04-11 [time] => 14:17:14 [challen_no] => chal713528 [indent_id] => ph001 [receiver_id] => PHC-ATHM [issuer_id] => ATHM [item_code] => Array ( [0] => ocp [1] => ocp ) [Item_type] => Array ( [0] => f [1] => p ) [item_quantity] => Array ( [0] => 10 [1] => 30 ) [qnt_issue] => Array ( [0] => 10 [1] => 30 ) [ids] => 2 ) 
$date=$_POST['date'];
$time=$_POST['time'];
	...
	// 	$qnt_issue_single=$qnt_issue[$i];
		...
		if($num_rows!=0){
			...
			$available_stock=$result_fetch['item_quantity'];
		 	$district_batch_id_stock=$result_fetch['block_stock_batch_id'];
		 	$remain_stock=$available_stock-$qnt_issue_single;
			if(0>$remain_stock){
				$issue_stock=$qnt_issue_single+($remain_stock);
				$qnt_issue1=$issue_stock;

				$query_batch="SELECT * FROM `rhc_master_stock_block_batch_details` WHERE `block_stock_batch_id`='$district_batch_id_stock' and `status`='1' and `date_exp`>'$date' ORDER BY YEAR(`date_exp`) ASC, MONTH(`date_exp`) ASC, DAY(`date_exp`) ASC";
				...
				if($num_rows1!=0){
					while ($res_batch=mysqli_fetch_assoc($sql_exe_batch)) {
						...
						if($issue_stocks<$qnt_issue1){
							if(0>$check_avaialble){
								...
								$item_details[]=array("district_batch_id_stock"=>"$district_batch_id_stock","item_code_single"=>"$item_code_single","Item_type_single"=>"$Item_type_single","batch_nos"=>"$batch_nos","remaining_quantity"=>"$remaining_quantity","date_exp"=>"$date_exp","remaining_quantity1"=>"$remaining_quantity1",'district_item_quantity'=>"$district_item_quantity",'slno'=>'$slno');
					...
					$item_details[]=array("district_batch_id_stock"=>"0","item_code_single"=>"$item_code_single","Item_type_single"=>"$Item_type_single","batch_nos"=>"0","remaining_quantity"=>"0","date_exp"=>"0",'remaining_quantity1'=>"0",'district_item_quantity'=>"$district_item_quantity" ,'slno'=>"0");
			 	}
			}else{
			 	$query_batch="SELECT * FROM `rhc_master_stock_block_batch_details` WHERE `block_stock_batch_id`='$district_batch_id_stock' and `status`='1' and `date_exp`>'$date' ORDER BY YEAR(`date_exp`) ASC, MONTH(`date_exp`) ASC, DAY(`date_exp`) ASC";
				$sql_exe_batch=mysqli_query($conn,$query_batch);
					...
					while ($res_batch=mysqli_fetch_assoc($sql_exe_batch)) {
						...
						if($issue_stocks<$qnt_issue_single){
							if(0>$check_avaialble){
								...
								$item_details[]=array("district_batch_id_stock"=>"$district_batch_id_stock","item_code_single"=>"$item_code_single","Item_type_single"=>"$Item_type_single","batch_nos"=>"$batch_nos","remaining_quantity"=>"$issue_stocks1","date_exp"=>"$date_exp","remaining_quantity1"=>"$remaining_quantity1",'district_item_quantity'=>"$district_item_quantity",'slno'=>"$slno");
							}
						}						
					}
				// echo $issue_stocks;
		...
		for ($i=0; $i <count($item_batch_array) ; $i++) { 
				...
				if($ty==$ty1){
					...
					if($district_stock_batch_id!="0"){
						...
						if($remain_qnt==0){
							...
							$stock_update_district="UPDATE `rhc_master_stock_block_batch_details` SET `remaining_quantity`='$remain_qnt',`status`='$status',`date_creation`='$date1',`time_creation`='$time1' WHERE `block_stock_batch_id`='$district_stock_batch_id' AND `batch_nos`='$batch_no' and `slno`='$slno' ";
						}else{
						 	$status="1";
							$stock_update_district="UPDATE `rhc_master_stock_block_batch_details` SET `remaining_quantity`='$remain_qnt',`status`='$status',`date_creation`='$date1',`time_creation`='$time1' WHERE `block_stock_batch_id`='$district_stock_batch_id' AND `batch_nos`='$batch_no' and `slno`='$slno'  ";
						}
			...
			if($district_item_quantity!="0"){
				...
				if($district_remain_qnt==0){
					$status1="2";
					$query_stock_item_update="UPDATE `rhc_master_stock_block_items` SET `item_quantity`='$district_remain_qnt',`date_creation`='$date1',`time_creation`='$time1',`status`='$status1' WHERE `block_stock_batch_id`='$district_stock_batch_id' and `item_codes`='$code_item' and `item_types`='$type_item' and `block_place_id`='$issuer_id'  ";
