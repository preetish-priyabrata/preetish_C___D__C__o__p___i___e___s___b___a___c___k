...
$total_cal_item=$total_item-$total+$total_batch;
	...
	if($total_cal_item==0){
		$state_update="UPDATE `rhc_master_stock_uphc_items` SET `item_quantity`='$total_cal_item',`status`='0' WHERE`uphc_stock_batch_id`='$batch_store_id'";
		$sql_exe=mysqli_query($conn,$state_update);
	}else{
		if($total_cal_item>0){
			$state_update="UPDATE `rhc_master_stock_uphc_items` SET `item_quantity`='$total_cal_item',`status`='1' WHERE`uphc_stock_batch_id`='$batch_store_id'";
			$sql_exe=mysqli_query($conn,$state_update);
		}else{
	...
	if($total_batch==0){
		$state_batch="UPDATE `rhc_master_stock_uphc_batch_details` SET `total_quantity`='$total_batch',`remaining_quantity`='$quanity',`status`='0',`remark`='$remark' WHERE`uphc_stock_batch_id`='$batch_store_id' and `slno`='$slno'";
		$sql_exe_batch=mysqli_query($conn,$state_batch);
	}else{
		if($total_batch>0){
			$state_batch="UPDATE `rhc_master_stock_uphc_batch_details` SET `total_quantity`='$total_batch',`remaining_quantity`='$quanity',`status`='1',`remark`='$remark' WHERE`uphc_stock_batch_id`='$batch_store_id' and `slno`='$slno'";
			$sql_exe_batch=mysqli_query($conn,$state_batch);
		}else{
