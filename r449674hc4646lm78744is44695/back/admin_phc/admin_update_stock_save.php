...
// print_r($updated_stock);
  ...
  for ($i=0; $i <$total ; $i++) {
    ...
    if($item_quantitys==0){
         ...
         $query_update="UPDATE `rhc_master_stock_phc_items` SET `item_quantity`='$item_quantitys',`date_creation`='$date',`time_creation`='$time',`status`='2' WHERE `slno`='$slnos'";
     
     
    }else{
      if($item_quantitys>0){
      
       $query_update="UPDATE `rhc_master_stock_phc_items` SET `item_quantity`='$item_quantitys',`date_creation`='$date',`time_creation`='$time',`status`='1' WHERE `slno`='$slnos'";
      }else{

         $query_update="UPDATE `rhc_master_stock_phc_items` SET `item_quantity`='0',`date_creation`='$date',`time_creation`='$time',`status`='2' WHERE `slno`='$slnos'";
      }
    }
   $sql_exe=mysqli_query($conn,$query_update);
  }
   $insert_query="INSERT INTO `rhc_master_update_phc_aphc_stock`(`slno`, `updated_place_to`, `updated_place_from`, `present_info`, `updated_info`, `date`, `time`, `updated_user_id`,`place_status`) VALUES (NULL,'$issuer_id','$receiver_id','$pres_stock','$updtd_stock','$date','$time','$user','1')";
