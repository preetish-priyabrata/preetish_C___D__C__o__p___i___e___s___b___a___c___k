...
include 'config.php';
 $msg = new \Preetish\FlashMessages\FlashMessages();
  ...
  $query_list="SELECT * FROM `rhc_master_phc_asha_indent` WHERE `indent_place_raised_to`='$place_id' and `indent_id`='$indent_id' and `status`!='0' ";
 ...
 if($result_list['place_status']=='1'){
    ...
    $get_phc1="SELECT * FROM `rhc_master_place_phc_sub_center` WHERE `place_phc_sub_id`='$receiver_id'";
    $sql_exec_phc1=mysqli_query($conn,$get_phc1);
    $phc_fetch_detail1=mysqli_fetch_assoc($sql_exec_phc1);                           
    $place_full_name=strtoupper($phc_fetch_detail1['phc_sub_center_name']).'['.$phc_fetch_detail1['place_phc_sub_id'].']';    
    }else{
  ...
  $place_full_name1= strtoupper($phc_fetch_detail['phc_name']).'['.$phc_fetch_detail['place_phc_id'].']'; 
