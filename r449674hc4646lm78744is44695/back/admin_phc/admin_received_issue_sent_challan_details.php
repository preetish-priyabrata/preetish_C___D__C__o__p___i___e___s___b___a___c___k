...
include 'config.php';
 $msg = new \Preetish\FlashMessages\FlashMessages();
  ...
  $query_challan="SELECT * FROM `rhc_master_phc_aphc_challan_no` WHERE `challen_no`='$challen' and  `status`='1'";
...
// exit;
  ...
  // $get_place_fetch_detail=mysqli_fetch_assoc($sql_exe_get_place);
   ...
   $query_list="SELECT * FROM `rhc_master_phc_aphc_indent` WHERE `indent_place_raised_by`='$place_id' and `indent_id`='$indent_id' AND `status`!='1'";
  ...
  // print_r($result_list);
  // exit;
  $place_id1=$result_fetch_challen['issuer_place_id'];
  $query_get_place="SELECT * FROM `rhc_master_place_block` WHERE `place_block_id`='$place_id1'";
  $sql_exe_get_place=mysqli_query($conn,$query_get_place);
  $district_fetch_detail=mysqli_fetch_assoc($sql_exe_get_place);
    $place_full_name1=strtoupper($district_fetch_detail['block_name']).'['.$district_fetch_detail['place_block_id'].']';
  
   if($result_list['place_status']=='1'){
    $receiver_id=$result_list['indent_place_raised_by'];
    $get_block="SELECT * FROM `rhc_master_place_phc` WHERE `place_phc_id`='$receiver_id'";
    $sql_exec_block=mysqli_query($conn,$get_block);
    $block_fetch_detail=mysqli_fetch_assoc($sql_exec_block);
    $place_full_name=strtoupper($block_fetch_detail['phc_name']).'['.$block_fetch_detail['place_phc_id'].']';
    }else{
      $receiver_id=$result_list['indent_place_raised_by'];
      $get_dh="SELECT * FROM `rhc_master_place_aphc` WHERE `place_aphc_id`='$receiver_id'";
      $sql_exec_dh=mysqli_query($conn,$get_dh);
      $dh_fetch_detail=mysqli_fetch_assoc($sql_exec_dh);
      $place_full_name=strtoupper($dh_fetch_detail['aphc_name']).'['.$dh_fetch_detail['place_aphc_id'].']';
    }
      ...
      <div id="section-to-print">
            <div class="col-md-12">
              ...
              <tbody align="center">
                  <?php 
                  $x=1;
                 $query_list_item="SELECT * FROM `rhc_master_phc_aphc_item_details_challan_no` WHERE `challan_no`='$challen' and `status`='1' ";
            ...
            <a href="admin_received_issue_sent_history.php"  class="btn btn-default">Back</a>
