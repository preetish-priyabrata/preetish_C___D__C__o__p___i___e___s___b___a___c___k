    ...
    function block($mobiles,$message1s){
    	  ...
    	  $message1=strtolower($sms_district);
    	    if(preg_match('/cancel indent/',$message1)){ // cancelation of indent
    	    	return block_cancel_indent($message1,$mobs);
    	   }else{
    	   		if (preg_match('/update/',$message1)){
    	   			...
    	   			if (preg_match('/cancel issue/',$message1)){ // cancelation of issue
    	   				...
    	   				return block_cancel_issue($message1,$mobs);
    	   			}else{
...
//////////////////////////////////////////////////////////////////////////////////////////////////
   	...
   	function block_update($message_ind,$mobs1){
    	global $conn;
	    ...
	    // here it is check by count where separates by comma
	     ...
	     if($simalar=="0"){
	     	for ($i=0; $i <$count_details ; $i++) { 
		     	...
		     	if($item_quantitys==0){      
         			$query_update="UPDATE `rhc_master_stock_block_items` SET `item_quantity`='$item_quantitys',`date_creation`='$date',`time_creation`='$time',`status`='2' WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `block_place_id`='$raise_place_id' ";
     				...
     				$array_list[]=$item_code[$i]." ".$item_quantity[$i]." ".$item_type[$i];
      				...
      				if($item_quantitys>0){      
       					$query_update="UPDATE `rhc_master_stock_block_items` SET `item_quantity`='$item_quantitys',`date_creation`='$date',`time_creation`='$time',`status`='1' WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `block_place_id`='$raise_place_id'";
       					$updated_stock[] = array('item_codes' =>$item_codes ,'item_types' =>$item_types,'item_quantity' =>$item_quantitys );
       					$array_list[]=$item_code[$i]." ".$item_quantity[$i]." ".$item_type[$i];
	     	...
	     	$query_get_stock="SELECT * FROM `rhc_master_stock_block_items` WHERE `block_place_id`='$raise_place_id'";
		    ...
		    $insert_query="INSERT INTO `rhc_master_update_dh_block_stock`(`slno`, `updated_place_to`, `updated_place_from`, `present_info`, `updated_info`, `place_status`, `date`, `time`, `updated_user_id`) VALUES (NULL,'$top_place_id','$raise_place_id','$pres_stock','$updtd_stock','1','$date','$time','$user')";
...
function block_iss_rec($message1,$mobs){
	...
	$check_challan="SELECT * FROM `rhc_master_dh_block_challan_no` WHERE `challen_no`='$values' and `receiver_place_id`='$place_block_dh_id' and  `status`='0' or `status`='2' ";
	$sql_exe_check_challan=mysqli_query($conn,$check_challan);
	 $num_rows=mysqli_num_rows($sql_exe_check_challan);

	$check_indent1="SELECT * FROM `rhc_master_phc_aphc_indent` WHERE `indent_id`='$values' and `indent_place_raised_to`='$place_block_dh_id' and  `status`='1'  ";
	...
	if(($num_rows==1) && ($num_rows1==0)){
		...
		return block_receive($message_value,$mobs_value,$challan_value,$item_details1,$receiver_id,$issuer_id);
		...
		$indent_value=$values;
		return block_issue($message_value,$mobs_value,$indent_value,$item_details1,$receiver_id,$issuer_id);
	}else{
...
//////////////////////////////////////////////////////////////////////////////////////////////////
//																								// 
// 			 							BLOCK RECEIVED SECTION 									//
// 																								//
//////////////////////////////////////////////////////////////////////////////////////////////////
// receiver section
function block_receive($message_value,$mobs_value,$challan_value,$item_details1,$receiver_id,$issuer_id){
	global $conn;
    ...
    if($simalar!="0"){
    	...
    	for ($i=0; $i <count($item_code); $i++) { 
	    	...
	    	$query_get_item_details="SELECT * FROM `rhc_master_dh_block_item_details_challan_no` WHERE `challan_no`='$challan_no' and `item_code`='$item_codes' And `item_type`='$item_types' And `quantity_issued`='$item_quantitys'	";
	    ...
	    if($cat!=0){
	    	echo "Invalid Entry Mismatch Values";
     		exit;
	    }else{
	    	for ($j=0; $j <count($item_code); $j++) { 
	    		...
	    		$array_list[]=$item_codes." ".$item_quantity_issues." ".$item_types;
		    	 ...
		    	 $query_update_item_challan="UPDATE `rhc_master_dh_block_item_details_challan_no` SET `quantity_received`='$item_quantity_issues',`status`='1' WHERE `challan_no`='$challan_no' and `item_code`='$item_codes' And `item_type`='$item_types'";
				...
				if($cal==0){
					$update="UPDATE `rhc_master_stock_block_items` SET `item_quantity`='$cal',`status`='2' WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `block_place_id`='$receiver_id'";
				}else{
		    ...
		    $query_get_item_details1="SELECT * FROM `rhc_master_dh_block_item_details_challan_no` WHERE `challan_no`='$challan_no' 	";
		    ...
		    for ($i=0; $i <count($batch_id_item_details) ; $i++) { 
			    ...
			    $query_batch_detail_item_challen="SELECT * FROM `rhc_master_dh_block_receive_batch_detail_item` WHERE `item_batch_id`='$item_batch_ids' and `item_type`='$item_types' and `item_code` ='$item_codes'";
			    ...
			    while ($res_batch_detail=mysqli_fetch_assoc($sql_exe_batch_details)) {
		    		...
		    		$$update_received_batches_info="UPDATE `rhc_master_dh_block_receive_batch_detail_item` SET `rec_batch_no`='$batch_no',`rec_batch_expire`='$date_expire',`rec_batch_quantity`='$batch_quantity' WHERE`slno`='$REC_BATCH'";
					$sql_exe_received_batches_info=mysqli_query($conn,$update_received_batches_info);

    				 $query_get_item_code_id1="SELECT * FROM `rhc_master_stock_block_items` WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `block_place_id`='$receiver_id'";
					...
					$block_check="SELECT * FROM `rhc_master_stock_block_batch_details` WHERE `block_stock_batch_id`='$block_stock_batch_id' and `batch_nos`='$batch_no'";
					  $sql_exe_check=mysqli_query($conn,$block_check);
					  ...
					  if($num_check_batch==0){
					    ...
					    if($remaining_qnt!=0){
					      $batch_update="UPDATE `rhc_master_stock_block_batch_details` SET `total_quantity`='$total_qnt',`remaining_quantity`='$remaining_qnt',`status`='1',`date_creation`='$date',`time_creation`='$time' WHERE  `slno`='$slno'";
			...
			$user_id=$mobs_value;
			$quer_challan_update="UPDATE `rhc_master_dh_block_challan_no` SET `date_update_status`='$date',`time_update_status`='$time',`user_id`='$user_id',`status`='1' WHERE `challen_no`='$challan_no'";
...
// 																								//
//////////////////////////////////////////////////////////////////////////////////////////////////
function block_issue($message_value,$mobs_value,$indent_value,$item_details1,$receiver_id,$issuer_id){
// starting of dist_issue {
	...
	$indent_id=$indent_no=$arr[0]; // defined indent id
	    	...
	    	if($simalar!="0"){
	    		...
	    		$query_get_detail="SELECT * FROM `rhc_master_phc_aphc_indent` WHERE `indent_id`='$indent_no' AND `status`='1'";
		    	...
		    	if($num_rows_get_details==0){
	    			...
	    			$user_name=$user=$result_fetch['user_id']; // user or user_name is defined	    			
	    			 $check_indent_status="SELECT * FROM `rhc_master_phc_aphc_indent` WHERE `indent_place_raised_to`='$issuer_id' and `indent_id`='$indent_no' AND `status`='1'";
		    		...
		    		if($num_rows_get_details1==0){ // checking valid indent id for for state or not..
						...
						$sql_check="SELECT * FROM `rhc_master_phc_aphc_challan_no` WHERE `indent_id`='$indent_id' and `status`!='5'";
						...
						if($num_indents!=0){
							...
							$query_challen="INSERT INTO `rhc_master_phc_aphc_challan_no`(`slno`, `receiver_place_id`, `issuer_place_id`, `indent_id`, `challen_no`, `date_creation`, `time_creation`, `issuer_id`,`status`) VALUES (NULL,'$receiver_id','$issuer_id' ,'$indent_id','$challen_no','$date','$time','$user_name','4')";
						...
						for ($i=0; $i <count($item_code) ; $i++) { 
							...
							$quer_item_update="UPDATE `rhc_master_phc_aphc_item_details_challan_no` SET `item_batch_id`='$item_batch_id' WHERE  `slno`='$last_id'";
						...
						// 	$qnt_issue_single=$qnt_issue[$i];
							...
							$query_check="SELECT * FROM `rhc_master_stock_block_items` WHERE `status`='1' AND `block_place_id`='$issuer_id' And `item_codes`='$item_code_single' And `item_types`='$Item_type_single'";
							...
							if($num_rows!=0){
								...
								$available_stock=$result_fetch['item_quantity'];
							 	$district_stock_batch_id=$result_fetch['block_stock_batch_id'];
							 	$remain_stock=$available_stock-$qnt_issue_single;
								if(0>$remain_stock){
									$issue_stock=$qnt_issue_single+($remain_stock);
									$qnt_issue1=$issue_stock;

									$query_batch="SELECT * FROM `rhc_master_stock_block_batch_details` WHERE `block_stock_batch_id`='$district_stock_batch_id' and `status`='1' and `date_exp`>'$date' ORDER BY YEAR(`date_exp`) ASC, MONTH(`date_exp`) ASC, DAY(`date_exp`) ASC";
					...
					//   exit;
							...
							for ($i=0; $i <count($item_batch_array) ; $i++) { 
										 	...
										 	$sql_exe_batch_insert=mysqli_query($conn,$query_store_batch);
												...
												if($sql_exe_batch_insert){
													...
													if($remain_qnt==0){
														...
														$stock_update_district="UPDATE `rhc_master_stock_block_batch_details` SET `remaining_quantity`='$remain_qnt',`status`='$status',`date_creation`='$date1',`time_creation`='$time1' WHERE `block_stock_batch_id`='$district_stock_batch_ids' AND `batch_nos`='$batch_no' and `slno`='$slno'";
													}else{
													 	$status="1";
														$stock_update_district="UPDATE `rhc_master_stock_block_batch_details` SET `remaining_quantity`='$remain_qnt',`status`='$status',`date_creation`='$date1',`time_creation`='$time1' WHERE `block_stock_batch_id`='$district_stock_batch_ids' AND `batch_nos`='$batch_no' and `slno`='$slno'";
													}
								...
								if($place_item_quantity!="0"){
									...
									if($stock_remain_qnt==0){
										$status1="2";
										$query_stock_item_update="UPDATE `rhc_master_stock_block_items` SET `item_quantity`='$stock_remain_qnt',`date_creation`='$date1',`time_creation`='$time1',`status`='$status1' WHERE `block_stock_batch_id`='$district_stock_batch_ids' and `item_codes`='$code_item' and `item_types`='$type_item' and ` 	block_place_id`='$issuer_id'  ";
							...
							$query_update_indent="UPDATE `rhc_master_phc_aphc_indent` SET `status`='2' WHERE `indent_id`='$indent_id' ";
							$sql_exe_update_indent=mysqli_query($conn,$query_update_indent);

						 	$query_update_status_challan="UPDATE `rhc_master_phc_aphc_challan_no` SET `status`='0' WHERE `challen_no`='$challen_no'";
							...
							$date=date('Y-m-d');
							$time=date('H:i:s');
							$get_item="SELECT * FROM `rhc_master_phc_aphc_item_details_challan_no` WHERE `challan_no`='$challen_no'";
							...
							// print_r($res);
						 	...
						 	$mobileno_top=implode(";",$array_list_user_mobile);
						  	...
						  	$query_list="SELECT * FROM `rhc_master_phc_aphc_indent` WHERE `indent_place_raised_to`='$place_id' and `indent_id`='$indent_id' ";
							...
							$result_list=mysqli_fetch_assoc($sql_exe_list);
					   		if($result_list['place_status']=='1'){
								...
								while($res_raise_level_mobile=mysqli_fetch_assoc($sql_exe_raise_level_mobile)){
	 									// print_r($res);
									$array_list_raise_user_mobile[]=$res_raise_level_mobile['user_mobile'];
									$district=$res_raise_level_mobile['place_district_id'];
								}
								$raise_level_mobile2="SELECT * FROM `rhc_master_login_user` WHERE `place_district_id`='$district' and `user_designation`='2' And `status`='1'";
						 		$sql_exe_top_level_mobile=mysqli_query($conn,$raise_level_mobile2);
						  		while($res_raise_level_mobile2=mysqli_fetch_assoc($sql_exe_top_level_mobile)){
						    		// print_r($res);
...
#																								#
#################################################################################################
function block_cancel_issue($message_ind,$mobs1){
	global $conn;
	 $message_ind;
	 $mobs1; // mobile no 
	$arr = explode(' ',trim($message_ind));
	 ...
	 $check_issued_challan="SELECT * FROM `rhc_master_phc_aphc_challan_no` WHERE `indent_id`='$indent_id' and `status`='0' or `status`='2'";
	...
	if($num_check_issued==0){
		...
		$cancel_query="INSERT INTO `rhc_master_cancel_histroy`(`slno`, `cancel_from`, `cancel_to`, `challan_no`, `indent_no`, `date_creation`, `time_creation`, `flag_place`) VALUES (NULL,'$issuer_id','$receiver_id','$challen_no','$indent_id','$date','$time','3')" ;
		...
		$query_list_item="SELECT * FROM `rhc_master_phc_aphc_item_details_challan_no` WHERE `challan_no`='$challen_no'";
                ...
                while ($res_item=mysqli_fetch_assoc($sql_exe_list_item)) {
					...
					$sql_stock_item="SELECT * FROM `rhc_master_stock_block_items` WHERE `item_codes`='$item_codes' And `item_types`='$Item_types' and `block_place_id`='$issuer_id'";//item_quantity
					...
					if($num_stock_item!=0){
						...
						if($qty_item==0){
							$update_stock="UPDATE `rhc_master_stock_block_items` SET `item_quantity`='0' ,`status`='2' WHERE `slno`='$slno'";
						}else{
							$update_stock="UPDATE `rhc_master_stock_block_items` SET `item_quantity`='$qty_item' ,`status`='1' WHERE `slno`='$slno'";
						}
						$sql_exe=mysqli_query($conn,$update_stock);
					}
					// detele in issue stock table
					$sql_detele="DELETE FROM `rhc_master_phc_aphc_item_details_challan_no` WHERE `item_batch_id`='$item_batch_ids' AND `challan_no`='$challen_no'";
                ...
                for ($i=0; $i <count($store_array) ; $i++) { 
					...
					// stock update
					 $sql_stock_item="SELECT * FROM `rhc_master_stock_block_items` WHERE `item_codes`='$item_code_batchs' And `item_types`='$Item_type_batchs' and `block_place_id`='$issuer_id'";//item_quantity
					...
					if($num_stock_item!=0){
						...
						$stock_fetch_item1=mysqli_fetch_assoc($sql_exe_stock_item1);
						 $block_stock_batch_id1=$stock_fetch_item1['block_stock_batch_id'];
	                	 $query_batch_details="SELECT * FROM `rhc_master_phc_aphc_receive_batch_detail_item` WHERE `item_batch_id`='$item_batch_id'";
		                ...
		                if($num_rows_batch!=0){
		                    while ($result_batch=mysqli_fetch_assoc($sql_exe_batch_deatils)) {
								...
								// 
								 ...
								 $stock_batch="SELECT * FROM `rhc_master_stock_block_batch_details` WHERE `block_stock_batch_id`='$block_stock_batch_id1' and `batch_nos`='$batch_nos'";
								...
								if($num_stock_item_batch==1){
									...
									if($qty_remaining==0){
										$update_stock_batch="UPDATE `rhc_master_stock_block_batch_details` SET `status`='2',`remaining_quantity`='0' WHERE `slno`='$slno'";
									}else{
										$update_stock_batch="UPDATE `rhc_master_stock_block_batch_details` SET `status`='1',`remaining_quantity`='$qty_remaining' WHERE `slno`='$slno'";
									}
								...
								// 
								$del_batch_issue_query="DELETE FROM `rhc_master_phc_aphc_receive_batch_detail_item` WHERE `item_batch_id`='$item_batch_idss' and `item_code`='$item_code_batchs' and `batch_no`='$batch_nos'";
		        ...
		        // 
		    $det_challan="DELETE FROM `rhc_master_phc_aphc_challan_no` WHERE `indent_id`='$indent_id' and `challen_no`='$challen_no'";
			$sql_exe=mysqli_query($conn,$det_challan);
			$det_update_sql="UPDATE `rhc_master_phc_aphc_indent` SET `status`='1' WHERE `indent_id`='$indent_id'";
			...
			while($res_top_level_mobile=mysqli_fetch_assoc($sql_exe_top_level_mobile)){
			 ...
			 $mobileno_top=implode(";",$array_list_user_mobile);
			  ...
			  $query_list="SELECT * FROM `rhc_master_phc_aphc_indent` WHERE `indent_place_raised_to`='$issuer_id' and `indent_id`='$indent_id' ";
		  $sql_exe_list=mysqli_query($conn,$query_list);
		$result_list=mysqli_fetch_assoc($sql_exe_list);
		   if($result_list['place_status']=='1'){
		    	$raise_level_mobile="SELECT * FROM `rhc_master_login_user` WHERE `place_phc_aphc_id`='$receiver_id' and `user_designation`='5' and `status`='1'";
...
#																								#
#################################################################################################
function block_cancel_indent($message_ind,$mobs1){
	global $conn;
	 $message_ind;
	 $mobs1; // mobile no 
	$arr = explode(' ',trim($message_ind));
	 ...
	 $indent_id=trim($arr[3]);
	 $check_issued_challan="SELECT * FROM `rhc_master_dh_block_indent` WHERE `indent_id` ='$indent_id' and `status`='1' ";
	$exe_check_issued=mysqli_query($conn,$check_issued_challan);
	 $num_check_issued=mysqli_num_rows($exe_check_issued);
	if($num_check_issued==0){
