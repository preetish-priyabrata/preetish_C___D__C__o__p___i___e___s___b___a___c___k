...
//////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function phc_indent($message_ind,$mobs1){
    	global $conn;
    	// include 'config.php';
    	$message_ind;
    	$mobs1;
	    ...
	    // here it is check by count where separates by comma
	     ...
	     if($res_ok==0){
			...
			$result_fetch=mysqli_fetch_assoc($sql_exe);
	    	 ...
	    	 $raise_place_id=$result_fetch['place_phc_aphc_id'];
	     ...
	     if($simalar=="0"){
  			...
  			$time= date('H:i:s');
  			$da=date('md');
   			$query_intent_details="INSERT INTO `rhc_master_phc_aphc_indent`(`slno`, `indent_place_raised_to`, `indent_place_raised_by`, `date_creation`, `time_creation`,`inserted_by_id`) VALUES (NULL,'$top_place_id','$raise_place_id' ,'$date','$time','$user')";
  				$sql_exe_insert=mysqli_query($conn,$query_intent_details);
  				
    				$last_id = mysqli_insert_id($conn);
    				$get_id="ph00".$da.$last_id;
     				$query_update_indent="UPDATE `rhc_master_phc_aphc_indent` SET `indent_id`='$get_id' WHERE `slno`='$last_id'";
		      			...
		      			for ($i=0; $i <$count_details ; $i++) { 
			        		...
			        		$item_codes=$item_code[$i];
			         		...
			         		$item_quantitys=$item_quantity[$i];
			          		...
			          		// echo "hi";
			           		$check_indent="SELECT * FROM `rhc_master_phc_aphc_indent_id_details` WHERE `indent_id`='$get_id' and `item_code`='$item_codes' and `Item_type`='$item_types'";
			         		 $sql_check=mysqli_query($conn,$check_indent);
			          		$num_check=mysqli_num_rows($sql_check); 
			          		if($num_check==0){        
			          			$query_item_insert="INSERT INTO `rhc_master_phc_aphc_indent_id_details`(`slno`, `indent_id`, `item_code`, `Item_type`, `item_quantity`, `date_creation`, `time_creation`) VALUES (NULL, '$get_id','$item_codes','$item_types','$item_quantitys','$date','$time')";
			  	...
			  	// 	$top_level_mobile="SELECT * FROM `rhc_master_login_user` WHERE `place_block_dh_id`='$top_place_id' and `user_designation`='3' And `status`='1'";
			   ...
			   //  		$array_list_user_mobile[]=$res_top_level_mobile['user_mobile'];
			 		...
			 		$array_list_user_mobile=array();
					  ...
					  while($res_top_level_mobile=mysqli_fetch_assoc($sql_exe_top_level_mobile)){
					    ...
					    $district=$res_top_level_mobile['place_district_id'];
					  }
					    // district
			    	...
			    	$mobileno_top=implode(";",$array_list_user_mobile);
			  		...
			  		$raise_level_mobile="SELECT * FROM `rhc_master_login_user` WHERE `place_phc_aphc_id`='$raise_place_id' and `user_designation`='5' And `status`='1'";
...
//////////////////////////////////////////////////////////////////////////////////////////////////
   	...
   	function phc_update($message_ind,$mobs1){
    	global $conn;
	    ...
	    // here it is check by count where separates by comma
	     ...
	     if($simalar=="0"){
	     	for ($i=0; $i <$count_details ; $i++) { 
		     	...
		     	if($item_quantitys==0){      
         			$query_update="UPDATE `rhc_master_stock_phc_items` SET `item_quantity`='$item_quantitys',`date_creation`='$date',`time_creation`='$time',`status`='2' WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `phc_place_id`='$raise_place_id' ";
     				...
     				$array_list[]=$item_code[$i]." ".$item_quantity[$i]." ".$item_type[$i];
      				...
      				if($item_quantitys>0){      
       					$query_update="UPDATE `rhc_master_stock_phc_items` SET `item_quantity`='$item_quantitys',`date_creation`='$date',`time_creation`='$time',`status`='1' WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `phc_place_id`='$raise_place_id'";
       					$updated_stock[] = array('item_codes' =>$item_codes ,'item_types' =>$item_types,'item_quantity' =>$item_quantitys );
       					$array_list[]=$item_code[$i]." ".$item_quantity[$i]." ".$item_type[$i];
	     	...
	     	$query_get_stock="SELECT * FROM `rhc_master_stock_phc_items` WHERE `phc_place_id`='$raise_place_id'";
		    ...
		    $insert_query="INSERT INTO `rhc_master_update_phc_aphc_stock`(`slno`, `updated_place_to`, `updated_place_from`, `present_info`, `updated_info`, `place_status`, `date`, `time`, `updated_user_id`) VALUES (NULL,'$top_place_id','$raise_place_id','$pres_stock','$updtd_stock','1','$date','$time','$user')";
...
// ///////////////////////////////////////////////////////////////////////////////////////////////
//																								// 
// 			  Issuce section and receive setion of PHC HERE IT SEPRATED				    		//
// 																								//
// ///////////////////////////////////////////////////////////////////////////////////////////////

function phc_iss_rec($message1,$mobs){
	...
	$sql_exe=mysqli_query($conn,$query_check);
	$result_fetch=mysqli_fetch_assoc($sql_exe);
	 $place_phc_aphc_id=$result_fetch['place_phc_aphc_id'];

	$check_challan="SELECT * FROM `rhc_master_phc_aphc_challan_no` WHERE `challen_no`='$values' and `receiver_place_id`='$place_phc_aphc_id' and  `status`='0' or `status`='2' ";
	$sql_exe_check_challan=mysqli_query($conn,$check_challan);
	 $num_rows=mysqli_num_rows($sql_exe_check_challan);

	$check_indent1="SELECT * FROM `rhc_master_phc_asha_indent` WHERE `indent_id`='$values' and `indent_place_raised_to`='$place_phc_aphc_id' and  `status`='1'  ";
	...
	if(($num_rows==1) && ($num_rows1==0)){
		...
		return phc_receive($message_value,$mobs_value,$challan_value,$item_details1,$receiver_id,$issuer_id);
		...
		$indent_value=$values;
		return phc_issue($message_value,$mobs_value,$indent_value,$item_details1,$receiver_id,$issuer_id);
	}else{
...
//////////////////////////////////////////////////////////////////////////////////////////////////
// receiver section
function phc_receive($message_value,$mobs_value,$challan_value,$item_details1,$receiver_id,$issuer_id){
	global $conn;
    ...
    if($simalar!="0"){
    	...
    	for ($i=0; $i <count($item_code); $i++) { 
	    	...
	    	$query_get_item_details="SELECT * FROM `rhc_master_phc_aphc_item_details_challan_no` WHERE `challan_no`='$challan_no' and `item_code`='$item_codes' And `item_type`='$item_types' And `quantity_issued`='$item_quantitys'	";
	    ...
	    if($cat!=0){
	    	...
	    	for ($j=0; $j <count($item_code); $j++) { 
	    		...
	    		$array_list[]=$item_codes." ".$item_quantity_issues." ".$item_types;
		    	 ...
		    	 $query_update_item_challan="UPDATE `rhc_master_phc_aphc_item_details_challan_no` SET `quantity_received`='$item_quantity_issues',`status`='1' WHERE `challan_no`='$challan_no' and `item_code`='$item_codes' And `item_type`='$item_types'";
					$sql_exe_update_item=mysqli_query($conn,$query_update_item_challan);
					// get quantity present
					$query_get_item_code_id="SELECT * FROM `rhc_master_stock_phc_items` WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `phc_place_id`='$receiver_id'";
				...
				$item_quantityss=$result_detail['item_quantity'];
				 ...
				 if($cal==0){	
					$update="UPDATE `rhc_master_stock_phc_items` SET `item_quantity`='$cal',`status`='1' WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `phc_place_id`='$receiver_id'";
				}else{
					$update="UPDATE `rhc_master_stock_phc_items` SET `item_quantity`='$cal',`status`='1' WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `phc_place_id`='$receiver_id'";
				}
				$sql_exe_update=mysqli_query($conn,$update);
		    }
		    $query_get_item_details1="SELECT * FROM `rhc_master_phc_aphc_item_details_challan_no` WHERE `challan_no`='$challan_no' 	";
		    ...
		    for ($i=0; $i <count($batch_id_item_details) ; $i++) { 
			    ...
			    $query_batch_detail_item_challen="SELECT * FROM `rhc_master_phc_aphc_receive_batch_detail_item` WHERE `item_batch_id`='$item_batch_ids' and `item_type`='$item_types' and `item_code` ='$item_codes'";
			    ...
			    while ($res_batch_detail=mysqli_fetch_assoc($sql_exe_batch_details)) {
		    		...
		    		$$update_received_batches_info="UPDATE `rhc_master_phc_aphc_receive_batch_detail_item` SET `rec_batch_no`='$batch_no',`rec_batch_expire`='$date_expire',`rec_batch_quantity`='$batch_quantity' WHERE`slno`='$REC_BATCH'";
					$sql_exe_received_batches_info=mysqli_query($conn,$update_received_batches_info);

    				 $query_get_item_code_id1="SELECT * FROM `rhc_master_stock_phc_items` WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `phc_place_id`='$receiver_id'";
					...
					$phc_stock_batch_id=$result_detail1['phc_stock_batch_id'];
					$dh_check="SELECT * FROM `rhc_master_stock_phc_batch_details` WHERE `phc_stock_batch_id`='$phc_stock_batch_id' and `batch_nos`='$batch_no'";
					  $sql_exe_check=mysqli_query($conn,$dh_check);
					  $num_check_batch=mysqli_num_rows($sql_exe_check);
					  	if($num_check_batch==0){
							if($batch_no!=0){
								$batch_insert="INSERT INTO `rhc_master_stock_phc_batch_details`(`slno`, `phc_stock_batch_id`, `batch_nos`, `date_exp`, `total_quantity`, `remaining_quantity`, `status`, `date_creation`, `time_creation`, `phc_place_id`) VALUES (NULL,'$phc_stock_batch_id','$batch_no','$date_expire','$batch_quantity','$batch_quantity','1','$date','$time','$receiver_id')";
						    ...
						    $total_qnt=$total_quantity+$batch_no;
						    $remaining_qnt=$remaining_quantity+$batch_no;
						    if($remaining_qnt!=0){
							    $batch_update="UPDATE `rhc_master_stock_phc_batch_details` SET `total_quantity`='$total_qnt',`remaining_quantity`='$remaining_qnt',`status`='1',`date_creation`='$date',`time_creation`='$time' WHERE  `slno`='$slno'";
			...
			$user_id=$mobs_value;
			$quer_challan_update="UPDATE `rhc_master_phc_aphc_challan_no` SET `date_update_status`='$date',`time_update_status`='$time',`user_id`='$user_id',`status`='1' WHERE `challen_no`='$challan_no'";
	...
	$code_details= implode(",",$array_list);
	// $top_level_mobile="SELECT * FROM `rhc_master_login_user` WHERE `place_block_dh_id`='$issuer_id' and `user_designation`='3' And `status`='1'";
	// $sql_exe_top_level_mobile=mysqli_query($conn,$top_level_mobile);
	// while($res_top_level_mobile=mysqli_fetch_assoc($sql_exe_top_level_mobile)){
	// 	// print_r($res);
	// 	$array_list_user_mobile[]=$res_top_level_mobile['user_mobile'];
	...
	// PHC
	 ...
	 $mobileno_top=implode(";",$array_list_user_mobile);
// /top detail info mobile no 
	 // lower level sms send
	 $raise_level_mobile="SELECT * FROM `rhc_master_login_user` WHERE `place_phc_aphc_id`='$receiver_id' and `user_designation`='5' And `status`='1'";
	$sql_exe_raise_level_mobile=mysqli_query($conn,$raise_level_mobile);
	while($res_raise_level_mobile=mysqli_fetch_assoc($sql_exe_raise_level_mobile)){
		// print_r($res);
		$array_list_raise_user_mobile[]=$res_raise_level_mobile['user_mobile'];
...
// 																								//
//////////////////////////////////////////////////////////////////////////////////////////////////
function phc_issue($message_value,$mobs_value,$indent_value,$item_details1,$receiver_id,$issuer_id){
// starting of dist_issue {
	...
	$get_id_place_status="SELECT * FROM `rhc_master_phc_asha_indent` WHERE `indent_id`='$indent_value'";
	$sql=mysqli_query($conn,$get_id_place_status);
	$place_status_fetch=mysqli_fetch_assoc($sql);
	$place_status_id=$place_status_fetch['place_status'];
	...
	$indent_id=$indent_no=$arr[0]; // defined indent id
	    	...
	    	if($simalar!="0"){
	    		...
	    		// here both phc_sub asha
	    		$query_get_detail="SELECT * FROM `rhc_master_phc_asha_indent` WHERE `indent_id`='$indent_no' AND `status`='1'";
		    	...
		    	if($num_rows_get_details==0){
	    			...
	    			$fetch_status=mysqli_fetch_assoc($sql_exe_get_details);
	    			$palce_status=$fetch_status['place_status'];
	    			...
	    			$user_name=$user=$result_fetch['user_id']; // user or user_name is defined	    			
	    			 $check_indent_status="SELECT * FROM `rhc_master_phc_asha_indent` WHERE `indent_place_raised_to`='$issuer_id' and `indent_id`='$indent_no' AND `place_status`='2' AND`status`='1'";
		    		...
		    		if($num_rows_get_details1==0){ // checking valid indent id for for state or not..
						...
						$sql_check="SELECT * FROM `rhc_master_phc_asha_challan_no` WHERE `indent_id`='$indent_id' and `status`!='5'";
						...
						if($num_indents!=0){
							...
							$query_challen="INSERT INTO `rhc_master_phc_asha_challan_no`(`slno`, `receiver_place_id`, `issuer_place_id`, `indent_id`, `challen_no`, `date_creation`, `time_creation`, `issuer_id`,`status`) VALUES (NULL,'$receiver_id','$issuer_id' ,'$indent_id','$challen_no','$date','$time','$user_name','4')";
						...
						for ($i=0; $i <count($item_code) ; $i++) { 
							...
							$quer_item_update="UPDATE `rhc_master_phc_asha_item_details_challan_no` SET `place_status`='$place_status_id', `item_batch_id`='$item_batch_id' WHERE  `slno`='$last_id'";
						...
						// 	$qnt_issue_single=$qnt_issue[$i];
							...
							$query_check="SELECT * FROM `rhc_master_stock_phc_items` WHERE `status`='1' AND `phc_place_id`='$issuer_id' And `item_codes`='$item_code_single' And `item_types`='$Item_type_single'";
							...
							if($num_rows!=0){
								...
								$available_stock=$result_fetch['item_quantity'];
							 	$phc_stock_batch_id=$result_fetch['phc_stock_batch_id'];
							 	$remain_stock=$available_stock-$qnt_issue_single;
								if(0>$remain_stock){
									$issue_stock=$qnt_issue_single+($remain_stock);
									$qnt_issue1=$issue_stock;

									$query_batch="SELECT * FROM `rhc_master_stock_phc_batch_details` WHERE `phc_stock_batch_id`='$phc_stock_batch_id' and `status`='1' and `date_exp`>'$date' ORDER BY YEAR(`date_exp`) ASC, MONTH(`date_exp`) ASC, DAY(`date_exp`) ASC";
									...
									if($num_rows1!=0){
										while ($res_batch=mysqli_fetch_assoc($sql_exe_batch)) {
											...
											if($issue_stocks<$qnt_issue1){
												if(0>$check_avaialble){
													...
													$item_details[]=array("phc_stock_batch_id"=>"$phc_stock_batch_id","item_code_single"=>"$item_code_single","Item_type_single"=>"$Item_type_single","batch_nos"=>"$batch_nos","remaining_quantity"=>"$remaining_quantity","date_exp"=>"$date_exp","remaining_quantity1"=>"$remaining_quantity1",'place_item_quantity'=>"$place_item_quantity",'slno'=>"$slnos");
										...
										$item_details[]=array("phc_stock_batch_id"=>"0","item_code_single"=>"$item_code_single","Item_type_single"=>"$Item_type_single","batch_nos"=>"0","remaining_quantity"=>"0","date_exp"=>"0",'remaining_quantity1'=>"0",'place_item_quantity'=>"$place_item_quantity",'slno'=>"0");
								 	}
								}else{
								 	$query_batch="SELECT * FROM `rhc_master_stock_phc_batch_details` WHERE `phc_stock_batch_id`='$phc_stock_batch_id' and `status`='1' and `date_exp`>'$date' ORDER BY YEAR(`date_exp`) ASC, MONTH(`date_exp`) ASC, DAY(`date_exp`) ASC";
									$sql_exe_batch=mysqli_query($conn,$query_batch);
					...
					//   exit;
							// store batch information in PHC tables AND UPDATE DISTICT BATCH STOCK  AS WELL AS DISTICT ITEM STOCK
							for ($i=0; $i <count($item_batch_array) ; $i++) { 
								for ($j=0; $j <count($item_details) ; $j++) { 
									...
									if($ty==$ty1){
										...
										$phc_stock_batch_ids=$item_details[$j]['phc_stock_batch_id'];
										 	...
										 	$sql_exe_batch_insert=mysqli_query($conn,$query_store_batch);
												...
												if($sql_exe_batch_insert){
													...
													if($remain_qnt==0){
														...
														$stock_update_district="UPDATE `rhc_master_stock_phc_batch_details` SET `remaining_quantity`='$remain_qnt',`status`='$status',`date_creation`='$date1',`time_creation`='$time1' WHERE `phc_stock_batch_id`='$phc_stock_batch_ids' AND `batch_nos`='$batch_no' and `slno`='$slno'";
													}else{
													 	$status="1";
														$stock_update_district="UPDATE `rhc_master_stock_phc_batch_details` SET `remaining_quantity`='$remain_qnt',`status`='$status',`date_creation`='$date1',`time_creation`='$time1' WHERE `phc_stock_batch_id`='$phc_stock_batch_ids' AND `batch_nos`='$batch_no' and `slno`='$slno'";
													}
								...
								if($place_item_quantity!="0"){
									...
									if($stock_remain_qnt==0){
										$status1="2";
										$query_stock_item_update="UPDATE `rhc_master_stock_phc_items` SET `item_quantity`='$stock_remain_qnt',`date_creation`='$date1',`time_creation`='$time1',`status`='$status1' WHERE `phc_stock_batch_id`='$phc_stock_batch_ids' and `item_codes`='$code_item' and `item_types`='$type_item' and ` 	phc_place_id`='$issuer_id'  ";
							...
							// print_r($res);
						 	...
						 	$mobileno_top=implode(";",$array_list_user_mobile);
						  	...
						  	$query_list="SELECT * FROM `rhc_master_phc_asha_indent` WHERE `indent_place_raised_to`='$place_id' and `indent_id`='$indent_id' ";
...
#																								#
#################################################################################################
function phc_cancel_issue($message_ind,$mobs1){
	global $conn;
	 $message_ind;
	 $mobs1; // mobile no 
	$arr = explode(' ',trim($message_ind));
	 ...
	 $check_issued_challan="SELECT * FROM `rhc_master_phc_asha_challan_no` WHERE `indent_id`='$indent_id' and `status`='0' or `status`='2'";
	...
	if($num_check_issued==0){
		...
		$cancel_query="INSERT INTO `rhc_master_cancel_histroy`(`slno`, `cancel_from`, `cancel_to`, `challan_no`, `indent_no`, `date_creation`, `time_creation`, `flag_place`) VALUES (NULL,'$issuer_id','$receiver_id','$challen_no','$indent_id','$date','$time','5')" ;
		...
		$query_list_item="SELECT * FROM `rhc_master_phc_asha_item_details_challan_no` WHERE `challan_no`='$challen_no'";
                ...
                while ($res_item=mysqli_fetch_assoc($sql_exe_list_item)) {
					...
					$sql_stock_item="SELECT * FROM `rhc_master_stock_phc_items` WHERE `item_codes`='$item_codes' And `item_types`='$Item_types' and `phc_place_id`='$issuer_id'";//item_quantity
					...
					if($num_stock_item!=0){
						...
						if($qty_item==0){
							$update_stock="UPDATE `rhc_master_stock_phc_items` SET `item_quantity`='0' ,`status`='2' WHERE `slno`='$slno'";
						}else{
							$update_stock="UPDATE `rhc_master_stock_phc_items` SET `item_quantity`='$qty_item' ,`status`='1' WHERE `slno`='$slno'";
						}
						$sql_exe=mysqli_query($conn,$update_stock);
					}
					// detele in issue stock table
					$sql_detele="DELETE FROM `rhc_master_phc_asha_item_details_challan_no` WHERE `item_batch_id`='$item_batch_ids' AND `challan_no`='$challen_no'";
                ...
                for ($i=0; $i <count($store_array) ; $i++) { 
					...
					// stock update
					 $sql_stock_item="SELECT * FROM `rhc_master_stock_phc_items` WHERE `item_codes`='$item_code_batchs' And `item_types`='$Item_type_batchs' and `phc_place_id`='$issuer_id'";//item_quantity
					...
					if($num_stock_item!=0){
						...
						$stock_fetch_item1=mysqli_fetch_assoc($sql_exe_stock_item1);
						 $phc_stock_batch_id=$stock_fetch_item1['phc_stock_batch_id'];
		                ...
		                if($num_rows_batch!=0){
		                    while ($result_batch=mysqli_fetch_assoc($sql_exe_batch_deatils)) {
								...
								// 
								 ...
								 $stock_batch="SELECT * FROM `rhc_master_stock_phc_batch_details` WHERE `phc_stock_batch_id`='$phc_stock_batch_id' and `batch_nos`='$batch_nos'";
								...
								if($num_stock_item_batch==1){
									...
									if($qty_remaining==0){
										$update_stock_batch="UPDATE `rhc_master_stock_phc_batch_details` SET `status`='2',`remaining_quantity`='0' WHERE `slno`='$slno'";
									}else{
										$update_stock_batch="UPDATE `rhc_master_stock_phc_batch_details` SET `status`='1',`remaining_quantity`='$qty_remaining' WHERE `slno`='$slno'";
									}
								...
								// 
								$del_batch_issue_query="DELETE FROM `rhc_master_phc_asha_receive_batch_detail_item` WHERE `item_batch_id`='$item_batch_idss' and `item_code`='$item_code_batchs' and `batch_no`='$batch_nos'";
		        ...
		        // 
		    $det_challan="DELETE FROM `rhc_master_phc_asha_challan_no` WHERE `indent_id`='$indent_id' and `challen_no`='$challen_no'";
			$sql_exe=mysqli_query($conn,$det_challan);
			$det_update_sql="UPDATE `rhc_master_phc_asha_indent` SET `status`='1' WHERE `indent_id`='$indent_id'";
...
#																								#
#################################################################################################
function phc_cancel_indent($message_ind,$mobs1){
	global $conn;
	 $message_ind;
	 $mobs1; // mobile no 
	$arr = explode(' ',trim($message_ind));
	 ...
	 $indent_id=trim($arr[3]);
	 $check_issued_challan="SELECT * FROM `rhc_master_phc_aphc_indent` WHERE `indent_id` ='$indent_id' and `status`='1' ";
	$exe_check_issued=mysqli_query($conn,$check_issued_challan);
	 $num_check_issued=mysqli_num_rows($exe_check_issued);
	if($num_check_issued==0){
		...
		$cancel_indent="UPDATE `rhc_master_phc_aphc_indent` SET `status`='0' WHERE `indent_id`='$indent_id'";
