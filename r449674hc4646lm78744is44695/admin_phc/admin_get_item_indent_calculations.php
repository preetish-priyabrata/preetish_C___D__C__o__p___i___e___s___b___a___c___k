...
// }else{
	$place_status=$_POST['place_status'];
	 if($place_status==5){
		...
		$query_table_received="SELECT SUM(`quantity_received`) FROM `rhc_master_phc_aphc_item_details_challan_no` WHERE `item_code`='$item_code' and `item_type`='$item_type' and `status`='1' And `receiver_place_id`='$place_id' And `date_creation` Between '$starting_date' AND '$ending_date'  ";
