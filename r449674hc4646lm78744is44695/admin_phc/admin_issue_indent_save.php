	...
	// 	$qnt_issue_single=$qnt_issue[$i];
		...
		if($num_rows!=0){
			...
			$available_stock=$result_fetch['item_quantity'];
		 	$phc_batch_id_stock=$result_fetch['phc_stock_batch_id'];
		 	$remain_stock=$available_stock-$qnt_issue_single;
			if(0>$remain_stock){
				$issue_stock=$qnt_issue_single+($remain_stock);
				$qnt_issue1=$issue_stock;

				$query_batch="SELECT * FROM `rhc_master_stock_phc_batch_details` WHERE `phc_stock_batch_id`='$phc_batch_id_stock' and `status`='1' and `date_exp`>'$date' ORDER BY YEAR(`date_exp`) ASC, MONTH(`date_exp`) ASC, DAY(`date_exp`) ASC";
		...
		for ($i=0; $i <count($item_batch_array) ; $i++) { 
			...
			if($state_item_quantity!="0"){
				...
				if($state_remain_qnt==0){
					$status1="2";
					$query_stock_item_update="UPDATE `rhc_master_stock_phc_items` SET `item_quantity`='$state_remain_qnt',`date_creation`='$date1',`time_creation`='$time1',`status`='$status1' WHERE `phc_stock_batch_id`='$phc_stock_batch_ids' and `item_codes`='$code_item' and `item_types`='$type_item' and `phc_place_id`='$issuer_id'  ";
