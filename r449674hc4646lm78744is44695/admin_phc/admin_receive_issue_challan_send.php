  ...
  $item=$array_lists[$j]['item'];
    ...
    if($x_item_batch==$x_item_rec){
      if($qnt!=0){
          ...
          $query_get_item_code_id1="SELECT * FROM `rhc_master_stock_phc_items` WHERE `item_types`='$Item_type_batchs' and `item_codes`='$item_code_batchs' and `phc_place_id`='$receiver_id'";
          ...
          $block_stock_batch_id=$result_detail1['phc_stock_batch_id'];
          $dh_check="SELECT * FROM `rhc_master_stock_phc_batch_details` WHERE `phc_stock_batch_id`='$block_stock_batch_id' and `batch_nos`='$batch_nos'";
          $sql_exe_check=mysqli_query($conn,$dh_check);
          $num_check_batch=mysqli_num_rows($sql_exe_check);
          if($num_check_batch==0){
            $batch_insert="INSERT INTO `rhc_master_stock_phc_batch_details`(`slno`, `phc_stock_batch_id`, `batch_nos`, `date_exp`, `total_quantity`, `remaining_quantity`, `status`, `date_creation`, `time_creation`, `phc_place_id`) VALUES (NULL,'$block_stock_batch_id','$batch_nos','$date_expires','$batch_quantitys','$batch_quantitys','1','$date','$time','$receiver_id')";
