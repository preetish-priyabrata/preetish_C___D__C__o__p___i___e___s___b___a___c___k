...
$result_fetch_challen =mysqli_fetch_assoc($sql_exe_challan); 
 ...
 $issuer_id=$place_id=$result_fetch_challen['issuer_place_id'];
   ...
   $query_list="SELECT * FROM `rhc_master_phc_asha_indent` WHERE `indent_place_raised_by`='$place_id1' and `indent_id`='$indent_id' and `status`='1'";
  ...
  while ($res_item=mysqli_fetch_assoc($sql_exe_list_item)) {
    ...
    $query_batch_details="SELECT * FROM `rhc_master_phc_asha_receive_batch_detail_item` WHERE `item_batch_id`='$item_batch_idm'";
 ...
 // print_r($batch_no);
  ...
  for ($i=0; $i <count($item_code) ; $i++) { 
    ...
    $sql_detele="DELETE FROM `rhc_master_phc_asha_item_details_challan_no` WHERE `item_batch_id`='$item_batch_ids' AND `challan_no`='$challen'";
  ...
  for ($i=0; $i <count($item_code) ; $i++) { 
    for ($j=0; $j <count($item_code_batch) ; $j++) { 
      ...
      if($ty==$ty1){
        ...
        if($num_stock_item!=0){
          $stock_fetch_item=mysqli_fetch_assoc($sql_exe_stock_item);
          $phc_stock_batch_id=$stock_fetch_item['phc_stock_batch_id'];
  ...
  $det_challan="DELETE FROM `rhc_master_phc_asha_challan_no` WHERE `indent_id`='$indent_id' and `challen_no`='$challen'";
