...
//////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function dh_indent($message_ind,$mobs1){
    	global $conn;
    	// include 'config.php';
    	$message_ind;
    	$mobs1;
	    ...
	    // here it is check by count where separates by comma
	     ...
	     if($simalar=="0"){
  			...
  			$time= date('H:i:s');
  			$da=date('md');
   			$query_intent_details="INSERT INTO `rhc_master_dh_block_indent`(`slno`, `indent_place_raised_to`, `indent_place_raised_by`,`place_status`, `date_creation`, `time_creation`,`inserted_by_id`) VALUES (NULL,'$top_place_id','$raise_place_id','2','$date','$time','$user')";
			    ...
			    // print_r($res);
			    	...
			    	$mobileno_top=implode(";",$array_list_user_mobile);
			  		$raise_level_mobile="SELECT * FROM `rhc_master_login_user` WHERE `place_block_dh_id`='$raise_place_id' and `user_designation`='4' And `status`='1'";
...
//////////////////////////////////////////////////////////////////////////////////////////////////
   	...
   	function dh_update($message_ind,$mobs1){
    	global $conn;
	    ...
	    // here it is check by count where separates by comma
	     ...
	     if($simalar=="0"){
	     	for ($i=0; $i <$count_details ; $i++) { 
		     	...
		     	if($item_quantitys==0){      
         			$query_update="UPDATE `rhc_master_stock_district_hospital_items` SET `item_quantity`='$item_quantitys',`date_creation`='$date',`time_creation`='$time',`status`='2' WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `dh_place_id`='$raise_place_id' ";
     				...
     				$array_list[]=$item_code[$i]." ".$item_quantity[$i]." ".$item_type[$i];
      				...
      				if($item_quantitys>0){      
       					$query_update="UPDATE `rhc_master_stock_district_hospital_items` SET `item_quantity`='$item_quantitys',`date_creation`='$date',`time_creation`='$time',`status`='1' WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `dh_place_id`='$raise_place_id'";
       					$updated_stock[] = array('item_codes' =>$item_codes ,'item_types' =>$item_types,'item_quantity' =>$item_quantitys );
       					$array_list[]=$item_code[$i]." ".$item_quantity[$i]." ".$item_type[$i];
	     	...
	     	$query_get_stock="SELECT * FROM `rhc_master_stock_district_hospital_items` WHERE `dh_place_id`='$raise_place_id'";
		    ...
		    $insert_query="INSERT INTO `rhc_master_update_dh_block_stock`(`slno`, `updated_place_to`, `updated_place_from`, `present_info`, `updated_info`, `place_status`, `date`, `time`, `updated_user_id`) VALUES (NULL,'$top_place_id','$raise_place_id','$pres_stock','$updtd_stock','2','$date','$time','$user')";
...
// ///////////////////////////////////////////////////////////////////////////////////////////////
//																								// 
// 			                  receive setion of Dh HERE IT SEPRATED	        					//
// 																								//
// ///////////////////////////////////////////////////////////////////////////////////////////////

function dh_iss_rec($message1,$mobs){
	...
	if(($num_rows==1) && ($num_rows1==0)){
		...
		return dh_receive($message_value,$mobs_value,$challan_value,$item_details1,$receiver_id,$issuer_id);

	}else if(($num_rows==0) && ($num_rows1==1)){
		echo "sorry Issue feature is not Avaliable with District Hospital";
		exit;
		// $fetch_get_details=mysqli_fetch_assoc($sql_exe_check_challan);
		// $receiver_id=$fetch_get_details['receiver_place_id'];
	 //    $issuer_id=$fetch_get_details['issuer_place_id'];
...
//////////////////////////////////////////////////////////////////////////////////////////////////
// receiver section
function dh_receive($message_value,$mobs_value,$challan_value,$item_details1,$receiver_id,$issuer_id){
	global $conn;
	    	...
	    	for ($j=0; $j <count($item_code); $j++) { 
	    		...
	    		$array_list[]=$item_codes." ".$item_quantity_issues." ".$item_types;
		    	 ...
		    	 $query_update_item_challan="UPDATE `rhc_master_dh_block_item_details_challan_no` SET `quantity_received`='$item_quantity_issues',`status`='1' WHERE `challan_no`='$challan_no' and `item_code`='$item_codes' And `item_type`='$item_types'";
					...
					$query_get_item_code_id="SELECT * FROM `rhc_master_stock_district_hospital_items` WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `dh_place_id`='$receiver_id'";
				...
				$update="UPDATE `rhc_master_stock_district_hospital_items` SET `item_quantity`='$cal',`status`='1' WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `dh_place_id`='$receiver_id'";
		    ...
		    for ($i=0; $i <count($batch_id_item_details) ; $i++) { 
			    ...
			    while ($res_batch_detail=mysqli_fetch_assoc($sql_exe_batch_details)) {
					...
					$sql_exe_received_batches_info=mysqli_query($conn,$update_received_batches_info);
    				 ...
    				 $query_get_item_code_id1="SELECT * FROM `rhc_master_stock_district_hospital_items` WHERE `item_types`='$item_types' and `item_codes`='$item_codes' and `dh_place_id`='$receiver_id'";
					...
					$dh_stock_batch_id=$result_detail1['dh_stock_batch_id'];
					$dh_check="SELECT * FROM `rhc_master_stock_district_hospital_batch_details` WHERE `dh_stock_batch_id`='$dh_stock_batch_id' and `batch_nos`='$batch_no'";
					  $sql_exe_check=mysqli_query($conn,$dh_check);
					  $num_check_batch=mysqli_num_rows($sql_exe_check);
					  if($num_check_batch==0){
						$batch_insert="INSERT INTO `rhc_master_stock_district_hospital_batch_details`(`slno`, `dh_stock_batch_id`, `batch_nos`, `date_exp`, `total_quantity`, `remaining_quantity`, `status`, `date_creation`, `time_creation`, `dh_place_id`) VALUES (NULL,'$dh_stock_batch_id','$batch_no','$date_expire','$batch_quantity','$batch_quantity','1','$date','$time','$receiver_id')";
						    ...
						    if($remaining_qnt!=0){
						      $batch_update="UPDATE `rhc_master_stock_district_hospital_batch_details` SET `total_quantity`='$total_qnt',`remaining_quantity`='$remaining_qnt',`status`='1',`date_creation`='$date',`time_creation`='$time' WHERE  `slno`='$slno'";
...
// 	    			echo $check_indent_status="SELECT * FROM `rhc_master_dh_block_indent` WHERE `indent_place_raised_to`='$issuer_id' and `indent_id`='$indent_no' AND `status`='1'";
// 		    		$sql_exe_get_details_status=mysqli_query($conn,$check_indent_status);
// 		    		echo $num_rows_get_details1=mysqli_num_rows($sql_exe_get_details_status);
...
#																								#
#################################################################################################
function dh_cancel_indent($message_ind,$mobs1){
	global $conn;
	$message_ind;
	$mobs1; // mobile no 
	$arr = explode(' ',trim($message_ind));
	 ...
	 $check_issued_challan="SELECT * FROM `rhc_master_dh_block_indent` WHERE `indent_id` ='$indent_id' and `status`='1' and `place_status`='2' ";
	...
	if($num_check_issued==0){
    	...
    	// district
		   ...
		   $raise_level_mobile="SELECT * FROM `rhc_master_login_user` WHERE `place_district_id`='$raise_place_id' and `user_designation`='4' And `status`='1'";
		  $sql_exe_raise_level_mobile=mysqli_query($conn,$raise_level_mobile);
		  while($res_raise_level_mobile=mysqli_fetch_assoc($sql_exe_raise_level_mobile)){
		    // print_r($res);
		    $array_list_raise_user_mobile[]=$res_raise_level_mobile['user_mobile'];
