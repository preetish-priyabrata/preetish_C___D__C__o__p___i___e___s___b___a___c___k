	 ...
	 <?php
      ...
      <table id="example1" class="table table-bordered table-striped table-hover text-center">
            ...
            <?php
              $x=1;
              $place_id1=$hsc_ids;
               $query_list="SELECT * FROM `rhc_master_phc_asha_indent` WHERE `indent_place_raised_by`='$hsc_ids' AND `status`!='0' and  `date_creation` BETWEEN '$date_one' AND '$date_two' order by `slno` desc";
              $sql_exe_list=mysqli_query($conn,$query_list);
              while($result_list=mysqli_fetch_assoc($sql_exe_list)){
            ?>
            <tr>
