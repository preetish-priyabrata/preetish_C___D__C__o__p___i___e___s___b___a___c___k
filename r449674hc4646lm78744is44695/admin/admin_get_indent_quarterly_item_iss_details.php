                ...
                $res_lis_type=mysqli_fetch_assoc($sql_exe_item_type);
                 ...
                 $item_names=$res_list['item_name'].'-'.$res_lis_type['item_type_name'];
                  ...
                  $query_list="SELECT * FROM rhc_master_phc_asha_indent_id_details INNER JOIN rhc_master_phc_asha_indent ON rhc_master_phc_asha_indent_id_details.indent_id=rhc_master_phc_asha_indent.indent_id WHERE rhc_master_phc_asha_indent.status!='0' and rhc_master_phc_asha_indent.indent_place_raised_by='$phc_aphc' and rhc_master_phc_asha_indent_id_details.item_code='$item_code' and rhc_master_phc_asha_indent_id_details.Item_type='$item_type'and  rhc_master_phc_asha_indent.date_creation BETWEEN '$date_one' AND '$date_two' ";
                    ...
                    $query_issued="SELECT SUM(quantity_issued),SUM(quantity_received) FROM `rhc_master_phc_asha_item_details_challan_no` WHERE `receiver_place_id`='$block_dh_uphc' and `item_code`='$item_code' and `item_type`='$item_type' and `date_creation` BETWEEN '$year_one' AND '$date_two'";

                    $query_issued_transit="SELECT SUM(quantity_issued) FROM `rhc_master_phc_asha_item_details_challan_no` WHERE `receiver_place_id`='$block_dh_uphc' and `status`!='1' and `item_code`='$item_code' and `item_type`='$item_type' and `date_creation` BETWEEN '$date_one' AND '$date_two'";
