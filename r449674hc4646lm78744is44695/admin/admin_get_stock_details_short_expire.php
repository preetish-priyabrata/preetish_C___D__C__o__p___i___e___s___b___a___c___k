                    ...
                    <table class="table table-striped text-center">
                      ...
                      <tbody>
                        <?php
                          $date_one=date('Y-m-d');
                          $date_two = date('Y-m-d', strtotime('+'.$nos_month.' month'));
                            $sql_query= "SELECT * FROM `rhc_master_stock_uphc_batch_details` WHERE  `date_exp`  BETWEEN '$date_one' AND '$date_two' and  `block_stock_batch_id`='$batch_ids' and `status`='1'";
                          $sql_exe_query=mysqli_query($conn,$sql_query);
                           $num=mysqli_num_rows($sql_exe_query);
                          if($num==0){?>
                            <tr>
                    ...
                    <table class="table table-striped text-center">
                      ...
                      <tbody>
                        <?php
                          $date_one=date('Y-m-d');
                          $date_two = date('Y-m-d', strtotime('+'.$nos_month.' month'));
                            $sql_query= "SELECT * FROM `rhc_master_stock_district_hospital_batch_details` WHERE  `date_exp`  BETWEEN '$date_one' AND '$date_two' and  `block_stock_batch_id`='$batch_ids' and `status`='1'";
                          $sql_exe_query=mysqli_query($conn,$sql_query);
                           $num=mysqli_num_rows($sql_exe_query);
                          if($num==0){?>
                            <tr>
      ...
      case '5';
            $hsc_ids=$hsc_ids;
              ...
              $x=0;
                ...
                // Commodity Name combine with name
                 $item_names=$res_list['item_name'].'-'.$res_lis_type['item_type_name'];
                 $batch_ids=$result_list_item['phc_sub_stock_batch_id'];
                  ?>
                 <tr>
                  <td><?=$x?></td>
                  <td><?=$item_names?></td>
                    ...
                    <table class="table table-striped text-center">
                      ...
                      <tbody>
                        <?php
                          $date_one=date('Y-m-d');
                          $date_two = date('Y-m-d', strtotime('+'.$nos_month.' month'));
                            $sql_query= "SELECT * FROM `rhc_master_stock_phc_subcenter_batch_details` WHERE `date_exp`  BETWEEN '$date_one' AND '$date_two' and  `phc_sub_stock_batch_id`='$batch_ids' and `status`='1'";
                          $sql_exe_query=mysqli_query($conn,$sql_query);
                           $num=mysqli_num_rows($sql_exe_query);
                          if($num==0){?>
                            <tr>
