			...
			<table id="example1" class="table table-bordered table-striped table-hover text-center">
		        ...
		        <?php
		          ...
		          $place_id1=$district_id;
		           $query_list="SELECT * FROM `rhc_master_district_indent` WHERE `indent_place_raised_by`='$place_id1' AND `status`!='0' and  YEAR(`date_creation`) BETWEEN '$year_one' AND '$year_two' order by `slno` desc";
	 ...
	 <?php
		...
		?>
			<h3 class="text-center">Indent Raised From <?=$place_name?></h3>
				<table id="example1" class="table table-bordered table-striped table-hover text-center">
                    ...
                    // indent_place_raised_to
                      ...
                      $query_list="SELECT * FROM `rhc_master_dh_block_indent` WHERE `indent_place_raised_by`='$place_ids' AND `status`!='0' and  YEAR(`date_creation`) BETWEEN '$year_one' AND '$year_two' order by `slno` desc";
                      $sql_exe_list=mysqli_query($conn,$query_list);
                      while($result_list=mysqli_fetch_assoc($sql_exe_list)){
                      	$x++;
                    ?>
			...
			?>
                    ...
                    // indent_place_raised_to
                      ...
                      $query_list="SELECT * FROM `rhc_master_phc_aphc_indent` WHERE `indent_place_raised_by`='$phc_aphc' AND `status`!='0' and  YEAR(`date_creation`) BETWEEN '$year_one' AND '$year_two' order by `slno` desc ";
                      $sql_exe_list=mysqli_query($conn,$query_list);
                      while($result_list=mysqli_fetch_assoc($sql_exe_list)){
                    ?>
                    <tr>
      ...
      <table id="example1" class="table table-bordered table-striped table-hover text-center">
            ...
            <?php
              $x=1;
              $place_id1=$hsc_ids;
               $query_list="SELECT * FROM `rhc_master_phc_asha_indent` WHERE `indent_place_raised_by`='$hsc_ids' AND `status`!='0' and  YEAR(`date_creation`) BETWEEN '$year_one' AND '$year_two' order by `slno` desc";
              $sql_exe_list=mysqli_query($conn,$query_list);
              while($result_list=mysqli_fetch_assoc($sql_exe_list)){
            ?>
            <tr>
              ...
              <td>
                <?php  
                     $place_phc_id=$result_list['indent_place_raised_to'];
                      $get_phc="SELECT * FROM `rhc_master_place_phc` WHERE `place_phc_id`='$place_phc_id'";
                      $sql_exec_block=mysqli_query($conn,$get_phc);
                      $block_fetch_detail=mysqli_fetch_assoc($sql_exec_block);
                      echo strtoupper($block_fetch_detail['phc_name']).'['.$block_fetch_detail['place_phc_id'].']';
                        ?>
              </td>
              <td><?=$result_list['indent_id'];?></td>
              <td><?=date('d-m-Y',strtotime(trim($result_list['date_creation'])));?></td>
              ...
              ?></td>
              <td>
              <a target="_blank" href="admin_raise_indent_hsc.php?indent_id=<?=$result_list['indent_id'];?>" class="btn btn-info" role="button">More</a></td>
