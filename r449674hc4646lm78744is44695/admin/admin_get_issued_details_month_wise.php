...
if($_SESSION['admin_emails']){
    ...
    $date_one=$year_one."-".$quarterly_selected."-"."1";
    $date_two=$year_two."-".$quarterly_selected."-"."31";
    $hsc_ids=trim($_POST['hsc_ids']);
		...
		switch ($place_id) {
			...
			?>
			<h3 class="text-center">Issued For <?=$place_name?></h3>
            ...
            <tr>
             <!--  <td><?=$x?></td> -->
              ...
              <td><?php if($num_rows_issued!=0){
                echo  $num_rows_issued; } ?></td>
              ...
              $x=1;
               ...
               $query_item="SELECT * FROM `rhc_master_stock_state_items` WHERE `state_place_id`='$state_id'";
                ...
                $res_lis_type=mysqli_fetch_assoc($sql_exe_item_type);
                 ...
                 $query_list1="SELECT SUM(rhc_master_phc_asha_indent_id_details.item_quantity) FROM rhc_master_phc_asha_indent_id_details INNER JOIN rhc_master_phc_asha_indent ON rhc_master_district_indent_id_details.indent_id=rhc_master_district_indent.indent_id WHERE rhc_master_phc_asha_indent.status!='0' and rhc_master_phc_asha_indent.indent_place_raised_by='$hsc_ids' and rhc_master_phc_asha_indent_id_details.item_code='$item_code' and rhc_master_phc_asha_indent_id_details.Item_type='$item_types' and  YEAR(rhc_master_phc_asha_indent.date_creation) BETWEEN '$date_one' AND '$date_two'";
                $sql_exe_list=mysqli_query($conn,$query_list1);
                $fetch=mysqli_fetch_array($sql_exe_list);

                 $query_issued="SELECT SUM(quantity_issued),SUM(quantity_received) FROM `rhc_master_phc_asha_item_details_challan_no` WHERE `receiver_place_id`='$district_id' and `item_code`='$item_code' and `item_type`='$item_types' and `date_creation` BETWEEN '$date_one' AND '$date_two'";

                $query_issued_transit="SELECT SUM(quantity_issued) FROM `rhc_master_phc_asha_item_details_challan_no` WHERE `receiver_place_id`='$district_id' and `status`!='1' and `item_code`='$item_code' and `item_type`='$item_types' and `date_creation` BETWEEN '$date_one' AND '$date_two'";
