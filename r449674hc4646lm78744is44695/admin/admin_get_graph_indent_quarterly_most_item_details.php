        ...
        $get_block="SELECT * FROM `rhc_master_place_district` ";
         ...
         $place_dist=array();
               ...
               $query_item="SELECT * FROM `rhc_master_stock_state_items` WHERE `state_place_id`='$state_id'";
                ...
                $array_item_name[]=$item_names;
                $array_freq_sort_before[]=$array_freq[]=mysqli_num_rows($sql_exe_list);

              }
             
              rsort($array_freq_sort_before);
              $arrlength = count($array_freq);
              for($x = 0; $x < $arrlength; $x++) {
                if($array_freq_sort_before[0]==$array_freq[$x]){
                  $datasets2[]=$array_freq[$x];
                  $item_names_find[]=$array_item_name[$x];
                }
              }
                 //print_r($datasets2);
