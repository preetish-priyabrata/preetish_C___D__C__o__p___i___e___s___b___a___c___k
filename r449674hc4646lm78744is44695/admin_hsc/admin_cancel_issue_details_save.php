...
// 
	...
	for ($i=0; $i <count($item_code) ; $i++) { 
		...
		$sql_stock_item="SELECT * FROM `rhc_master_stock_phc_subcenter_items` WHERE `item_codes`='$item_codes' And `item_types`='$Item_types' and `phc_sub_place_id`='$issuer_id'";//item_quantity
		...
		if($num_stock_item!=0){
			...
			if($qty_item==0){
				$update_stock="UPDATE `rhc_master_stock_phc_subcenter_items` SET `item_quantity`='0' ,`status`='2' WHERE `slno`='$slno'";
			}else{
				$update_stock="UPDATE `rhc_master_stock_phc_subcenter_items` SET `item_quantity`='$qty_item' ,`status`='1' WHERE `slno`='$slno'";
			}
	...
	for ($i=0; $i <count($item_code) ; $i++) { 
		for ($j=0; $j <count($item_code_batch) ; $j++) { 
			...
			if($ty==$ty1){
				...
				// stock update
				$sql_stock_item="SELECT * FROM `rhc_master_stock_phc_subcenter_items` WHERE `item_codes`='$item_code_batchs' And `item_types`='$Item_type_batchs' and `phc_sub_place_id`='$issuer_id'";//item_quantity
				$sql_exe_stock_item=mysqli_query($conn,$sql_stock_item);
				$num_stock_item=mysqli_num_rows($sql_exe_stock_item);
				if($num_stock_item!=0){
					$stock_fetch_item=mysqli_fetch_assoc($sql_exe_stock_item);
					$phc_sub_stock_batch_id=$stock_fetch_item['phc_sub_stock_batch_id'];
					$stock_batch="SELECT * FROM `rhc_master_stock_phc_subcenter_batch_details` WHERE `phc_sub_stock_batch_id`='$phc_sub_stock_batch_id' and `batch_nos`='$batch_nos'";
					...
					if($num_stock_item_batch==1){
						...
						if($qty_remaining==0){
							$update_stock_batch="UPDATE `rhc_master_stock_phc_subcenter_batch_details` SET `status`='2',`remaining_quantity`='0' WHERE `slno`='$slno'";
						}else{
							$update_stock_batch="UPDATE `rhc_master_stock_phc_subcenter_batch_details` SET `status`='1',`remaining_quantity`='$qty_remaining' WHERE `slno`='$slno'";
						}
	...
	$det_challan="DELETE FROM `rhc_master_asha_challan_no` WHERE `indent_id`='$indent_id' and `challen_no`='$challen_no'";
	$sql_exe=mysqli_query($conn,$det_challan);
	$det_update_sql="UPDATE `rhc_master_asha_indent` SET `status`='1' WHERE `indent_id`='$indent_id'";
	...
	while($res_raise_level_mobile=mysqli_fetch_assoc($sql_exe_raise_level_mobile)){
		...
		$place_phc_aphc_id=$res_raise_level_mobile['place_phc_aphc_id'];
								 ...
								 // PHC
	$raise_level_mobile31="SELECT * FROM `rhc_master_login_user` WHERE `place_phc_aphc_id`='$place_phc_aphc_id' and `user_designation`='5' And `status`='1'";
	$sql_exe_raise_level_mobile31=mysqli_query($conn,$raise_level_mobile31);
	while($res_raise_level_mobile31=mysqli_fetch_assoc($sql_exe_raise_level_mobile31)){
		$array_list_raise_user_mobile[]=$res_raise_level_mobile31['user_mobile'];
		$PHC=$res_raise_level_mobile31['place_block_dh_id'];
	}
	 // Block
