...
require 'config.php';
 $msg = new \Preetish\FlashMessages\FlashMessages();
    $place_phc_id=$_SESSION['place_id']; 
  $get_phc="SELECT * FROM `rhc_master_place_phc_sub_center` WHERE `place_phc_sub_id`='$place_phc_id'";
...
<link rel="stylesheet" href="//code.jquery.com/ui/1.12.1/themes/base/jquery-ui.css">
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        View Stock Of <?php  echo strtoupper($phc_fetch_detail['phc_sub_center_name']).'['.$phc_fetch_detail['place_phc_sub_id'].']';?>
      </h1>
      <ol class="breadcrumb">
        ...
        <div class="demo">
          <div class="panel-body">
              ...
              <tr>
               ...
               <?php 
               $date=date('Y-m-d');
                  $get_list="SELECT * FROM `rhc_master_stock_phc_subcenter_items` WHERE `phc_sub_place_id`='$place_phc_id' ";
                ...
                <?php if($res['status']==0){
                 ...
                 <table class="table" border="1">
                   ...
                   <?php 
                    $batch_id=$res['phc_sub_stock_batch_id'];
                    $get_batch="SELECT * FROM `rhc_master_stock_phc_subcenter_batch_details` WHERE `phc_sub_stock_batch_id`='$batch_id' and `status`='1' and `date_exp`>'$date' ORDER BY YEAR(`date_exp`) ASC, MONTH(`date_exp`) ASC, DAY(`date_exp`) ASC";
                ...
                <?php   }else{
                     ...
                     <tr>
                      ...
                      <!-- `slno``state_stock_batch_id``challan_received_id``batch_nos` -->
                       <td><a href="admin_edit_batch_info_current.php?slno=<?=$res_batch['slno']?>&batch_store_id=<?=$res_batch['phc_sub_stock_batch_id']?>&batch_no=<?=$res_batch['batch_nos']?>"><b style="color:green">Edit</b></a> </td>
                     </tr>
                     <?php }
                 }
                   ?>
