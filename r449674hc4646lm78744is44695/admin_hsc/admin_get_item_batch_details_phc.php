...
if($_POST['qnt_issue']!="" && $_POST['place_id']!="" && $_POST['itemtypes']!="" && $_POST['itemcodes']!=""){
	...
	$query_item="SELECT * FROM `rhc_master_stock_phc_subcenter_items` WHERE `status`='1' AND `phc_sub_place_id`='$place_id' And `item_codes`='$item_code' And `item_types`='$item_type'";
	...
	if($num_rows!=0){
		$result_fetch=mysqli_fetch_assoc($sql_exe_item);
		 ...
		 $item_batch_id=$result_fetch['phc_sub_stock_batch_id'];
		 $remain_stock=$available_stock-$qnt_issue;
		if(0>$remain_stock){
			$issue_stock=$qnt_issue+($remain_stock);
			$qnt_issue1=$issue_stock;

			$query_batch="SELECT * FROM `rhc_master_stock_phc_subcenter_batch_details` WHERE `phc_sub_stock_batch_id`='$item_batch_id' and `status`='1' and `date_exp`>'$date' ORDER BY YEAR(`date_exp`) ASC, MONTH(`date_exp`) ASC, DAY(`date_exp`) ASC";
