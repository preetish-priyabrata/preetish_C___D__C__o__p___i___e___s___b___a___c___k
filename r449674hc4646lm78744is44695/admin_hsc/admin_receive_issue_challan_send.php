...
for ($i=0; $i <count($item_code) ; $i++) { 
  ...
  $query_update_item_challan="UPDATE `rhc_master_phc_asha_item_details_challan_no` SET `quantity_received`='$quantity_receivedss',`status`='1' WHERE `challan_no`='$challen_no' and `item_code`='$item_codes' And `item_type`='$Item_types'";
...
$quer_challan_update="UPDATE `rhc_master_phc_asha_challan_no` SET `date_update_status`='$date',`time_update_status`='$time',`user_id`='$user_id',`status`='1',`comment`='$comment' WHERE `challen_no`='$challen_no'";
...
for ($i=0; $i <count($item_code_batch) ; $i++) { 
  ...
  $item=$array_lists[$j]['item'];
    ...
    if($x_item_batch==$x_item_rec){
      if($qnt!=0){
          ...
          $update_received_batches_info="UPDATE `rhc_master_phc_asha_receive_batch_detail_item` SET `rec_batch_no`='$batch_nos',`rec_batch_expire`='$date_expires',`rec_batch_quantity`='$batch_quantitys' WHERE`slno`='$item_batch_serial'";
          $sql_exe_received_batches_info=mysqli_query($conn,$update_received_batches_info);

          $query_get_item_code_id1="SELECT * FROM `rhc_master_stock_phc_subcenter_items` WHERE `item_types`='$Item_type_batchs' and `item_codes`='$item_code_batchs' and `phc_sub_place_id`='$receiver_id'";
          ...
          $dh_check="SELECT * FROM `rhc_master_stock_phc_subcenter_batch_details` WHERE `phc_sub_stock_batch_id`='$phc_sub_stock_batch_id' and `batch_nos`='$batch_nos'";
          $sql_exe_check=mysqli_query($conn,$dh_check);
          $num_check_batch=mysqli_num_rows($sql_exe_check);
          if($num_check_batch==0){
            $batch_insert="INSERT INTO `rhc_master_stock_phc_subcenter_batch_details`(`slno`, `phc_sub_stock_batch_id`, `batch_nos`, `date_exp`, `total_quantity`, `remaining_quantity`, `status`, `date_creation`, `time_creation`, `phc_sub_place_id`) VALUES (NULL,'$phc_sub_stock_batch_id','$batch_nos','$date_expires','$batch_quantitys','$batch_quantitys','1','$date','$time','$receiver_id')";
            ...
            $sql_exe_batch_insert=mysqli_query($conn,$batch_insert);
              ...
              if($remaining_qnt!=0){
                $batch_update="UPDATE `rhc_master_stock_phc_subcenter_batch_details` SET `total_quantity`='$total_qnt',`remaining_quantity`='$remaining_qnt',`status`='1',`date_creation`='$date',`time_creation`='$time' WHERE  `slno`='$slno'";
  ...
  while($res_top_level_mobile=mysqli_fetch_assoc($sql_exe_top_level_mobile)){
    ...
    $array_list_user_mobile[]=$res_top_level_mobile['user_mobile'];
     $place_block_dh_id=$res_top_level_mobile['place_block_dh_id'];
  }
  //block
    $top_level_mobile3="SELECT * FROM `rhc_master_login_user` WHERE `place_block_dh_id`='$issuer_id' and `user_designation`='3' and `status`='1' ";
  $sql_exe_top_level_mobile=mysqli_query($conn,$top_level_mobile3);
