...
// print_r($updated_stock);
  ...
  for ($i=0; $i <$total ; $i++) {
    ...
    if($item_quantitys==0){
         ...
         $query_update="UPDATE `rhc_master_stock_phc_subcenter_items` SET `item_quantity`='$item_quantitys',`date_creation`='$date',`time_creation`='$time',`status`='2' WHERE `slno`='$slnos'";
     
     
    }else{
      if($item_quantitys>0){
      
       $query_update="UPDATE `rhc_master_stock_phc_subcenter_items` SET `item_quantity`='$item_quantitys',`date_creation`='$date',`time_creation`='$time',`status`='1' WHERE `slno`='$slnos'";
      }else{

         $query_update="UPDATE `rhc_master_stock_phc_subcenter_items` SET `item_quantity`='0',`date_creation`='$date',`time_creation`='$time',`status`='2' WHERE `slno`='$slnos'";
      }
    }
   $sql_exe=mysqli_query($conn,$query_update);
  }
   $insert_query="INSERT INTO `rhc_master_update_phc_subcenter_stock`(`slno`, `updated_place_to`, `updated_place_from`, `present_info`, `updated_info`, `date`, `time`, `updated_user_id`,`place_status`) VALUES (NULL,'$issuer_id','$receiver_id','$pres_stock','$updtd_stock','$date','$time','$user','1')";
...
// sms
  ...
  $array_list_user_mobile=array();
   $top_level_mobile="SELECT * FROM `rhc_master_login_user` WHERE `place_phc_aphc_id`='$receiver_id' and `user_designation`='5' And `status`='1'";
  ...
  // block
  $top_level_mobile3="SELECT * FROM `rhc_master_login_user` WHERE `place_block_dh_id`='$place_block_dh_id' and `user_designation`='3' And `status`='1'";
 ...
 $raise_level_mobile="SELECT * FROM `rhc_master_login_user` WHERE `sub_center_id`='$receiver_id' and `user_designation`='7' and `status`='1'";
  ...
  if($server_output){
   ...
   // $mobileno=$result_fetch1['user_mobile'];
     ...
     $new_message1='Stock Updation Confirmation Sent Success-fully '.$datetime;
  ...
  // receive server response ...
    ...
    curl_close ($ch1);
    if($server_output1){
      $msg->success('Updated Submited  Success-fully');
