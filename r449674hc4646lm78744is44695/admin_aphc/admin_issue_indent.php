...
include 'config.php';
 $msg = new \Preetish\FlashMessages\FlashMessages();
  ...
  $query_list="SELECT * FROM `rhc_master_aphc_asha_indent` WHERE `indent_place_raised_to`='$place_id' and `indent_id`='$indent_id' AND `status`='1'";
  ...
  if($result_list['place_status']=='1'){
    ...
    $place_full_name=strtoupper($phc_fetch_detail['aphc_sub_center_name']).'['.$phc_fetch_detail['place_aphc_sub_id'].']'; 
    }else{
      ...
      // =strtoupper($aphc_fetch_detail['aphc_name']).'['.$aphc_fetch_detail['place_aphc_id'].']';
    }
  $place_id_block=$place_id;
  $get_block="SELECT * FROM `rhc_master_place_aphc` WHERE `place_aphc_id`='$place_id_block'";
  $sql_exec_block=mysqli_query($conn,$get_block);
  $block_fetch_detail=mysqli_fetch_assoc($sql_exec_block);
  
  $place_full_name1= strtoupper($block_fetch_detail['aphc_name']).'['.$block_fetch_detail['place_aphc_id'].']';
  ...
  while(1){        
        ...
        if($rowCount < 1){
          $challen_no=$appno;
          $query="INSERT INTO `rhc_master_challan_no_creation`(`slno`, `challen_no`, `place_issuer_id`, `place_issue_to`, `date_creation`, `time_creation`) VALUES (NULL,'$challen_no','$place_id' ,'$receiver_id','$date1','$time1')";
  ...
  <div class="content-wrapper">
    ...
    <div class="col-md-7">
      ...
      <div class="panel panel-default">
        <div class="panel-heading text-center" style="background-color: lightpink;"><strong>Issue Form</strong></div>
        <div class="demo">
              ...
              <tbody>
                ...
                <tr>
                    ...
                    <td><input type="text" name="qnt_issue[]" onclick="get_batch(<?=$x?>)" id="qnt_issue<?=$x?>" value="<?=$res_item['item_quantity']?>">
                      <input type="hidden" name="qnt_issued[]" onclick="get_batch(<?=$x?>)" id="qnt_issued<?=$x?>" value="<?=$res_item['item_quantity']?>">
                    <br>
  ...
  if(qnt_issue!=""){
    if(parseInt(qnt_issue)>parseInt(item_quantity)){
      ...
      document.getElementById("error"+id).innerHTML = "Please Insert a value less  Than or equalto ("+item_quantity+")";
       ...
       $.ajax({
                type:'POST',
                url:'admin_get_item_batch_details_phc.php',
  ...
  if(qnt_issues!=""){
    if(parseInt(item_quantitys)>=parseInt(qnt_issues)){
      document.getElementById("error"+id).innerHTML = "";
       $.ajax({
                type:'POST',
                url:'admin_get_item_batch_details_phcs.php',
  ...
  if(parseInt(check_ajax)>parseInt(ides)){
    ...
    for (var i = 1; i <= (ides); i++) {
        var qnt_issues=$('#qnt_issue'+i).val();
         var qnt_issueds=$('#qnt_issued'+i).val();
        if(qnt_issues!=""){
          if (parseInt(qnt_issues) > 0){
            if(parseInt(qnt_issues)>parseInt(qnt_issueds)){
              document.getElementById("error"+i).innerHTML = "Insert a value less than or equal to value of Indent quantity ";
            che_count++;
            errors++;
            }else{
