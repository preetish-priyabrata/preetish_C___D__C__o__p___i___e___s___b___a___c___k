...
for ($i=0; $i <count($item_code) ; $i++) { 
  ...
  $query_update_item_challan="UPDATE `rhc_master_phc_aphc_item_details_challan_no` SET `quantity_received`='$quantity_receivedss',`status`='1' WHERE `challan_no`='$challen_no' and `item_code`='$item_codes' And `item_type`='$Item_types'";
...
$quer_challan_update="UPDATE `rhc_master_phc_aphc_challan_no` SET `date_update_status`='$date',`time_update_status`='$time',`user_id`='$user_id',`status`='1',`comment`='$comment' WHERE `challen_no`='$challen_no'";
  ...
  $item=$array_lists[$j]['item'];
    ...
    if($x_item_batch==$x_item_rec){
      if($qnt!=0){
          ...
          $update_received_batches_info="UPDATE `rhc_master_phc_aphc_receive_batch_detail_item` SET `rec_batch_no`='$batch_nos',`rec_batch_expire`='$date_expires',`rec_batch_quantity`='$batch_quantitys' WHERE`slno`='$item_batch_serial'";
          $sql_exe_received_batches_info=mysqli_query($conn,$update_received_batches_info);

          $query_get_item_code_id1="SELECT * FROM `rhc_master_stock_aphc_items` WHERE `item_types`='$Item_type_batchs' and `item_codes`='$item_code_batchs' and `aphc_place_id`='$receiver_id'";
          ...
          $block_stock_batch_id=$result_detail1['aphc_stock_batch_id'];
          $dh_check="SELECT * FROM `rhc_master_stock_aphc_batch_details` WHERE `aphc_stock_batch_id`='$block_stock_batch_id' and `batch_nos`='$batch_nos'";
          $sql_exe_check=mysqli_query($conn,$dh_check);
          $num_check_batch=mysqli_num_rows($sql_exe_check);
          if($num_check_batch==0){
            $batch_insert="INSERT INTO `rhc_master_stock_phc_batch_details`(`slno`, `aphc_stock_batch_id`, `batch_nos`, `date_exp`, `total_quantity`, `remaining_quantity`, `status`, `date_creation`, `time_creation`, `aphc_place_id`) VALUES (NULL,'$block_stock_batch_id','$batch_nos','$date_expires','$batch_quantitys','$batch_quantitys','1','$date','$time','$receiver_id')";
