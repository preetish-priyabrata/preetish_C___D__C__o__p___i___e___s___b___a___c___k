...
// Array ( [date] => 2017-04-06 [time] => 18:25:37 [challen_no] => chal501348 [indent_id] => dis0004061 [receiver_id] => Pat [issuer_id] => BR [item_code] => Array ( [0] => ocp [1] => ocp ) [Item_type] => Array ( [0] => f [1] => p ) [item_quantity] => Array ( [0] => 10 [1] => 20 ) [qnt_issue] => Array ( [0] => 10 [1] => 20 ) [ids] => 2 ) 
$date1=date('Y-m-d');
$time1=date('H:i:s');
...
if($num_indents==0){
	...
	// 	$qnt_issue_single=$qnt_issue[$i];
		...
		$query_check="SELECT * FROM `rhc_master_stock_aphc_items` WHERE `status`='1' AND `aphc_place_id`='$issuer_id' And `item_codes`='$item_code_single' And `item_types`='$Item_type_single'";
		...
		if($num_rows!=0){
			...
			$available_stock=$result_fetch['item_quantity'];
		 	$phc_batch_id_stock=$result_fetch['aphc_stock_batch_id'];
		 	$remain_stock=$available_stock-$qnt_issue_single;
			if(0>$remain_stock){
				$issue_stock=$qnt_issue_single+($remain_stock);
				$qnt_issue1=$issue_stock;

				$query_batch="SELECT * FROM `rhc_master_stock_aphc_batch_details` WHERE `aphc_stock_batch_id`='$phc_batch_id_stock' and `status`='1' and `date_exp`>'$date' ORDER BY YEAR(`date_exp`) ASC, MONTH(`date_exp`) ASC, DAY(`date_exp`) ASC";
				...
				if($num_rows1!=0){
					while ($res_batch=mysqli_fetch_assoc($sql_exe_batch)) {
						...
						if($issue_stocks<$qnt_issue1){
							if(0>$check_avaialble){
								...
								$item_details[]=array("phc_stock_batch_id"=>"$phc_batch_id_stock","item_code_single"=>"$item_code_single","Item_type_single"=>"$Item_type_single","batch_nos"=>"$batch_nos","remaining_quantity"=>"$remaining_quantity","date_exp"=>"$date_exp","remaining_quantity1"=>"$remaining_quantity1",'state_item_quantity'=>"$state_item_quantity",'slno'=>"$slnos");
					...
					$item_details[]=array("phc_stock_batch_id"=>"0","item_code_single"=>"$item_code_single","Item_type_single"=>"$Item_type_single","batch_nos"=>"0","remaining_quantity"=>"0","date_exp"=>"0",'remaining_quantity1'=>"0",'state_item_quantity'=>"$state_item_quantity",'slno'=>"0");
			 	}
			}else{
			 	$query_batch="SELECT * FROM `rhc_master_stock_aphc_batch_details` WHERE `aphc_stock_batch_id`='$phc_batch_id_stock' and `status`='1' and `date_exp`>'$date' ORDER BY YEAR(`date_exp`) ASC, MONTH(`date_exp`) ASC, DAY(`date_exp`) ASC";
				$sql_exe_batch=mysqli_query($conn,$query_batch);
		...
		for ($i=0; $i <count($item_batch_array) ; $i++) { 
					 	...
					 	$sql_exe_batch_insert=mysqli_query($conn,$query_store_batch);
							...
							if($sql_exe_batch_insert){
								...
								if($remain_qnt==0){
									...
									$stock_update_district="UPDATE `rhc_master_stock_aphc_batch_details` SET `remaining_quantity`='$remain_qnt',`status`='$status',`date_creation`='$date1',`time_creation`='$time1' WHERE `aphc_stock_batch_id`='$phc_stock_batch_ids' AND `batch_nos`='$batch_no' and `slno`='$slno'";
								}else{
								 	$status="1";
									$stock_update_district="UPDATE `rhc_master_stock_aphc_batch_details` SET `remaining_quantity`='$remain_qnt',`status`='$status',`date_creation`='$date1',`time_creation`='$time1' WHERE `aphc_stock_batch_id`='$phc_stock_batch_ids' AND `batch_nos`='$batch_no' and `slno`='$slno'";
								}
			...
			if($state_item_quantity!="0"){
				...
				if($state_remain_qnt==0){
					$status1="2";
					$query_stock_item_update="UPDATE `rhc_master_stock_aphc_items` SET `item_quantity`='$state_remain_qnt',`date_creation`='$date1',`time_creation`='$time1',`status`='$status1' WHERE `aphc_stock_batch_id`='$phc_stock_batch_ids' and `item_codes`='$code_item' and `item_types`='$type_item' and `aphc_place_id`='$issuer_id'  ";

				}else{
					$status1="1";
					$query_stock_item_update="UPDATE `rhc_master_stock_phc_items` SET `item_quantity`='$state_remain_qnt',`date_creation`='$date1',`time_creation`='$time1',`status`='$status1' WHERE `phc_stock_batch_id`='$phc_stock_batch_ids' and `item_codes`='$code_item' and `item_types`='$type_item' and `aphc_place_id`='$issuer_id'  ";
