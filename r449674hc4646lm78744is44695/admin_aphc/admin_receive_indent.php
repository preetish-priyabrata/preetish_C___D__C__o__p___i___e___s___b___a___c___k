          ...
          <div class="table-responsive">
              <table id="example1" class="table table-bordered table-striped table-hover text-center">
                    ...
                    <?php
                      ...
                      $query_list="SELECT * FROM `rhc_master_aphc_asha_indent` WHERE `indent_place_raised_to`='$place_id' AND `status`='1'";
                      ...
                      <td>
                        <?php 
                          if($result_list['place_status']==1){
                            $place_aphc_sub_id=$result_list['indent_place_raised_by'];
                            $get_block="SELECT * FROM `rhc_master_place_aphc_sub_center` WHERE `place_aphc_sub_id`='$place_aphc_sub_id'";
                            $sql_exec_block=mysqli_query($conn,$get_block);
                            $block_fetch_detail=mysqli_fetch_assoc($sql_exec_block);
                            echo strtoupper($block_fetch_detail['aphc_sub_center_name']).'['.$block_fetch_detail['place_aphc_sub_id'].']';
                          }else{
