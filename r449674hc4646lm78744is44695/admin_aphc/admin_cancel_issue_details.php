...
$query_challan="SELECT * FROM `rhc_master_aphc_asha_challan_no` WHERE `challen_no`='$challen'";
...
$result_fetch_challen =mysqli_fetch_assoc($sql_exe_challan); 
 ...
 $place_id1=$result_fetch_challen['receiver_place_id'];
  ...
  $query_list="SELECT * FROM `rhc_master_aphc_asha_indent` WHERE `indent_place_raised_by`='$place_id1' and `indent_id`='$indent_id' and `status`='2'";
  ...
  //   exit;
  // }
  $get_block="SELECT * FROM `rhc_master_place_aphc` WHERE `place_aphc_id`='$place_id'";
  ...
  $result_list=mysqli_fetch_assoc($sql_exe_list);
    ...
    $place_full_name=strtoupper($phc_fetch_detail['aphc_sub_center_name']).'['.$phc_fetch_detail['place_phc_sub_id'].']'; 
    }else{
      ...
      <div class="panel panel-default">
        ...
        <div class="demo">
          <div class="panel-body">
            <form class="form-inline" action="admin_cancel_issue_details_save.php" method="POST" >
        <div  id="printarea">
      <div id="section-to-print">
            <div class="col-md-12">
              ...
              <tbody align="center">
                  <?php 
                  $x=1;
                $query_list_item="SELECT * FROM `rhc_master_aphc_asha_item_details_challan_no` WHERE `challan_no`='$challen'";
                 ...
                 <tr>
                    ...
                    <td>
                      <table class="table table-striped" border="1" width="100%">
                        ...
                        <tbody>
                        <input type="hidden" name="item_batch_id[]" id="item_batch_id<?=$x?>" value="<?=$res_item['item_batch_id']?>">
                        <?php 
                        ...
                        $item_batch_id=$res_item['item_batch_id'];
                          ...
                          if($num_rows_batch!=0){
                            ...
                            <input type="hidden" name="item_batch_idss[]" id="item_batch_idss<?=$x?>" value="<?=$res_item['item_batch_id']?>">
                             <tr>
           ...
           <button type="submit" class="btn btn-success pull-right" onclick="return confirm('Are you sure you want to Cancel this Challan?');">Cancel Challan</button>
          </form>
        </div>
      </div>
      </div>
