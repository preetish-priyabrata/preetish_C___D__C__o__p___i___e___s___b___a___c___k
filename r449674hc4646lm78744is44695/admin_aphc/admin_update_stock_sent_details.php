...
require 'config.php';
 ...
 $query_item="SELECT * FROM `rhc_master_update_phc_aphc_stock` WHERE `updated_place_from`='$place_id_district' and `slno`='$slno'";
            ...
            <div  id="printarea">
              ...
              <div class="form-group pull-right">
                ...
                <input type="hidden" name="place_status" id="place_status" value="<?=$_SESSION['designation']?>">
                                <?php 
                $get_district="SELECT * FROM `rhc_master_place_aphc`  WHERE `place_aphc_id`='$place_id_district'";
                $sql_exec_district=mysqli_query($conn,$get_district);
                $district_fetch_detail=mysqli_fetch_assoc($sql_exec_district);
                echo strtoupper($district_fetch_detail['aphc_name']).'['.$district_fetch_detail['place_aphc_id'].']'?>
