...
require 'config.php';
 ...
 $query_item="SELECT * FROM `rhc_master_update_aphc_asha_stock` WHERE `updated_place_to`='$place_id_district' and `slno`='$slno'";
...
if($list_details['place_status']=='1'){
    ...
    $get_phc="SELECT * FROM `rhc_master_place_aphc_sub_center` WHERE `place_aphc_sub_id`='$phc_id'";
    $sql_exec_phc=mysqli_query($conn,$get_phc);
    $phc_fetch_detail=mysqli_fetch_assoc($sql_exec_phc);
    $place_detail= strtoupper($phc_fetch_detail['aphc_sub_center_name']).'['.$phc_fetch_detail['place_aphc_sub_id'].']';                      
  }else{
      $place_detail=$place_id_aphc=$result_list['updated_place_from'];
