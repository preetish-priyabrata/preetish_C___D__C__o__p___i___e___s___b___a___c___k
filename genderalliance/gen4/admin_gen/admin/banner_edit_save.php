...
$dest="../upload/";
  if(!empty($_FILES['photo_name']['name'])){
    ...
    if(move_uploaded_file($_FILES['photo_name']['tmp_name'],"$dest".$file_name)){
       ...
       $query = "UPDATE `gen_banner` SET `short_desc`='$short_desc',`photo_name`='$file_name' where `sl_no`='$sl_no'";
          // echo $query;
      // echo $query1 = "SELECT * FROM  `gen_banner` where `sl_no`='$sl_no' AND `photo_name`='$file_name'";
          ...
          if($query_exe){
            ...
            header("location:banner_edit.php?sl_no=".$sl_no);
            exit;
          }else{
     ...
     $query = "UPDATE `gen_banner` SET `short_desc`='$short_desc' where `sl_no`='$sl_no'";
     $query_exe = mysqli_query($conn,$query);
