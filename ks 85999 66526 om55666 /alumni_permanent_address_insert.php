...
//insert personal profile
 $insert_query="UPDATE `master_permanent_address_profile` SET `at`='$at',`post`='$post',`via`='$via',`ps`='$PS',`city`='$city',`district`='$District',`pin`='$pin',`state`='$State',`zone`='$zone',`country`='$country',`land_ph_no`='$Landline',`date`='$date',`time`='$time' WHERE `user_id`='$user_id'";
//echo $insert_query;
...
if($sql_exe)
		{
           ...
           $flag="UPDATE `profile_table_flag` SET `permanent_address`='1' WHERE `user_id`='$user_id'";
			$sql_exe_flag=mysqli_query($conn,$flag);
			$update="UPDATE `master_user_registration` SET `update_profile`='7' WHERE `reg_no`='$reg_no'";
			$sql_exe_update=mysqli_query($conn,$update);
			header('Location:logout.php');
			exit;
			
			 $msg->success('Your Parmanent Address SuccessFull Submitted');
