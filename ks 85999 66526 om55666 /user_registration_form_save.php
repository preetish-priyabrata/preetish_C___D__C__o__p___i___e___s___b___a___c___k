...
if(isset($_POST['submit'])){
	...
	if(!empty($_FILES['img']['name'])){
		...
		if(move_uploaded_file($_FILES['img']['tmp_name'],"$dest".$file_name)){
			$sql_query="INSERT into `master_user_registration`(`name`,`email`,`Mobile_no`,`pass_year`,`stream`,`password`,`reg_no`,`photo`) values('$name','$email','$mobile','$pass_year','$stream','$password','$reg_no','$file_name')";

  			$sql_exe=mysqli_query($conn,$sql_query); 			// echo mysqli_error($conn);
