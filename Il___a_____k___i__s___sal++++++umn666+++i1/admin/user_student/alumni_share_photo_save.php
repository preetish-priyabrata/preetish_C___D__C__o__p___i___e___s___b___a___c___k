	...
	if(!empty($_FILES['img']['name'])){
		...
		if(move_uploaded_file($_FILES['img']['tmp_name'],"$dest".$file_name)){
			...
			$last_id=mysqli_insert_id($conn);
			 ...
			 $query_student1="INSERT INTO `user_sharing_info_details`(`sl_no`, `user_id`, `title`, `sharing_info_id`, `photo`,`text`,`post_type`, `date`, `time_entry`) VALUES (NULL,'$email','$title','$last_id','$file_name','$text','4','$date','$time')";

			 $sql_exe1=mysqli_query($conn,$query_student1);
			//exit;
			// echo mysqli_error($conn);
			//exit();
			$msg->success('Photo/Text Sent Successfull');
		...
		$msg->error('Photo Or Text Is Not matched');
		header('Location:alumni_dashboard.php');
		exit();
	}
