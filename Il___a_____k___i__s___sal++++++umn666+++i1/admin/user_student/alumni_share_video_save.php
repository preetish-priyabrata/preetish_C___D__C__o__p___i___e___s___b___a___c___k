...
if($_SESSION['email']){
	...
	if (isset($_POST['submit']))
   {
	...
	// $sharing_info_id=$_POST['$last_id'];
			 ...
			 $query_student1="INSERT INTO `user_sharing_info_details`(`sl_no`, `user_id`, `title`,`sharing_info_id`,`url`, `post_type`, `date`,`time_entry`) VALUES (NULL,'$email','$title','$last_id','$url','6','$date','$time')";
			 $sql_exe1=mysqli_query($conn,$query_student1);
			 //echo mysqli_error($conn);
			//exit();
          if($sql_exe1)
              {
              $msg->success('Video Sent Successfully');
