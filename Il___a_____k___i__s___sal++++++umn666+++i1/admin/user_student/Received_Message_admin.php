          ...
          <table id="example1" class="table table-bordered table-striped text-center">
            ...
            <tbody>
              ...
              while($rec2=mysqli_fetch_array($sql_exe)){    
                ...
                if($rec2['view_status']=="0"){     
              ?>
              <tr class="success">
                <td><?php echo $y ;?></td>
                <td><?php $post_id=$rec2['post_id'];
                         $sql_post="SELECT * FROM `admin_admin_post_temp` WHERE `slno`='$post_id'";
                         $sql_exe_post=mysqli_query($conn,$sql_post);
                         $fetch_post=mysqli_fetch_assoc($sql_exe_post);
                         $var=$fetch_post['text_message'];
                ...
                <td>  <a class="btn btn-primary btn-xs" href="Received_Message_view_admin.php?serial=<?=$rec2['slno']?>&receiver=<?=$sl_no_one?>&ids_uni=<?=$rec2['post_id']?>">View Message</a> </td>
              </tr>
