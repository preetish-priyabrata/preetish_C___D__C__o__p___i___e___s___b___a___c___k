...
<?php   
    if(!empty($_SESSION['exam_selected'])){
      	...
      	$qry_check="SELECT * FROM `pre_exam_candidate` where `exam_name`='$exam'";
      	$sql_check=mysqli_query($conn,$qry_check);
      	$num_rows_check=mysqli_num_rows($sql_check);
      	if($num_rows_check==0){?>
      		 <div class="col-md-12">
		        <div class="alert alert-success text-center">
		          <strong >No Roll Number Is Generated For Exam <?=$exam?></strong>
		        </div>
      		</div>
...
<?php
	        ...
	        if($num_rows==0){?>
	        	<div class="col-md-12">
			        <div class="alert alert-success text-center">
			          <strong >All Roll Number Is Assigned To Center For Exam <?=$exam?></strong>
			        </div>
	      		</div>
			       		...
			       		<form class="form-horizontal" method="POST" action="save_roll_center.php" role="form">
			       			<div class="form-group">
				       			...
				       			<select id="center" name="center" class="form-control" required>
									  ...
									  //$qry_centre="SELECT * FROM `center_master_data`  ";
											...
											if($num_rows==0){
												$qry_centre="SELECT center_master_data.slno,center_master_data.examcenter,center_master_data.sitting_capacity FROM center_master_data";
											}else{
												$qry_centre="SELECT c.examcenter,c.slno,c.sitting_capacity FROM center_master_data c left JOIN assign_exam_center a on a.center_name=c.examcenter and a.exam_name='$exam' and c.status='1' WHERE  a.center_name is NULL  ";
											}
									  		$sql_centre=mysqli_query($conn, $qry_centre);
									 	echo $sql_ce=mysqli_num_rows($sql_centre);
							...
							<div class="form-group">
								<label class="control-label col-sm-2" for="email">Roll NO From:</label>
								 <div class="col-sm-10">
								<input type="text" readonly class="form-control" name="from_roll" id="from_roll" value="<?=$red['roll_no']?>">
								</div>
							</div>
							<!--<div class="form-group">
