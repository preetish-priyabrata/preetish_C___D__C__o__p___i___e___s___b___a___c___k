...
<?php
        ...
        if($res_admit['update_attendance_exam']==1){
            $qry_checks="SELECT * FROM `assign_exam_center` where `exam_name`='$exam' AND `generated_admit_card`='1'And `update_attendance_exam`='1' And `mark_entry_status`='1'";
          ...
          ?>
              <div class="col-md-12">
            <div class="alert alert-success text-center">
              <strong > Please Enter Marks Is Entered  For The : <?=$exam?> </strong>
            </div>
          </div>
           <?php }else{

           ?>
                <form class="form-horizontal" action="postexam_mark_view.php" method="POST" role="form">
      ...
      <div class="container">
        <form action="post_exam_save_marks.php" method="POST" >
          ...
          <table class="table table-striped ">
            ...
            <tbody>
<?php         
          $qry_application="SELECT * FROM `post_exam_center_mark_roll` where `exam_name`='$exam' AND `center_name`='$center' ";
          $i=0;//absent
          $y=0;//present
