	...
	<div class="col-lg-10">
		<div class="panel panel-default">
		   <div class="table-responsive">          
			    ...
			    <tbody>
			      <?php 
				   		...
				   		while ($results=mysqli_fetch_assoc($sql_category)) { 
				   			...
				   			?>
				   	<form action="postexam_save_intimation.php" method="post">
				  	<tr>
				      	<td scope="row"><?=$i?>.</td>
				      	<td  scope="row">
				      	...
				      	<td >
				      		...
				      		?>
				      		<input type="hidden"  name="total_qualfi[]"  value="<?=$results['total_qualfi']?>">
				      		</div>
				      	</td>
				      	<?php if($results['total_app']!="0"){?>
				      	<td  scope="row"><input type="text" name="rank[]" id="rank<?=$i?>" onchange="call_total(<?=$i?>)" required></td>
				      	<td scope="row"><input type="text" name="no_call[]" required id="no_call<?=$i?>" readonly required></td>
				      	<?php }else{ ?>
				      		<td  scope="row"><input type="hidden" name="rank[]" id="rank<?=$i?>" value="0"></td>
				      		<td scope="row"><input type="hidden" name="no_call[]" id="no_call<?=$i?>" value="0"></td>
				      		<?php } ?>
				    </tr>
				    <?php }?>
  			...
  			<div class="text-center">
		    <input type="submit" class="btn btn-success" value="Update Intimation">
		    </div>
		   </form>
		 </div>
	</div>
