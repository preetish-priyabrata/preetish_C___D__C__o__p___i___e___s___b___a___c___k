...
if($_SESSION['final_verification_exam'])
{
	...
	$qry="UPDATE `user_master_data` SET `password`='$newpass',`datetime`=NOW() WHERE `username`='$admin_name' AND `usertype`='final verification'";
	
	$res= mysqli_query($conn, $qry);
	if($res){
		$msg->success('You have Change Your Password  Success-Fully ');
    	header("Location:final_verification_change_password.php");
    	exit;
	}else{
