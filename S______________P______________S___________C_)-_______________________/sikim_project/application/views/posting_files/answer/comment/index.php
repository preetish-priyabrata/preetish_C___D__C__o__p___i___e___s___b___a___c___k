           ...
           <?php $adats=array('answer_id'=>$id);
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                    ...
                    <?php 
                      $x=0;
                     $datas=array('del_status'=>0,'answer_id'=>$id);
                     $this->db->order_by('id', 'DESC');
                      $query=$this->db->get_where('q_a_comment',$datas);
                    ...
                    ?>
                      ...
                      <tr>
                        ...
                        <?php 
                         ...
                         <td> <a class="btn btn-success"  href="<?=base_url()?>view-abuse-answer/<?=$key->comment_ans_id?>"><?=$key->scan_no?></a></td>
                        <td><?=$key->comment?></td>
                         <td><?=$key->date?></td>
                        <td><?=$key->time?></td>
                      <td>
                                            <?php
                                               if ($key->status == 1) {
                                            ?>
                                                            <a href="<?php echo site_url('update-answer-comment-status/2/' . $key->id.'/'.$id) ?>" name='active' class='badge bg-blue' style='cursor:pointer;'>Active</a>
                                                    <?php
                                                  } else {
                                                   ?>
                                                            <a href='<?php echo site_url('update-answer-comment-status/1/' . $key->id.'/'.$id) ?>' name='inactive' class='badge bg-red' style='cursor:pointer;'>Inactive</a>
                                                    <?php
}
  ?>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
       <form class="form-horizontal form-label-left" action="<?=base_url()?>deletes-answer-comment" method="POST" role="form">
