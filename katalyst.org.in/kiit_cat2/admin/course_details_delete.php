...
if (isset($_GET['course'])) {
    $programDelete = str_replace(" ", "+", $_GET['course']);
    $programId = decryptIt_webs($programDelete);
    $status_id=decryptIt_webs(str_replace(" ", "+",$_GET['status']));
    if($status_id==3){
    	$queryUpdateProgram = "UPDATE `tbl_course` SET `status` = '3' WHERE `sl_no` = '$programId'";
		    $resUpdateProgram = mysqli_query($conn, $queryUpdateProgram);
		    if ($resUpdateProgram) {
			    $msg->success('Successfull Program Is Started ');
				header('Location:course_details_view.php');
				exit;
			}else{
			...
			$queryUpdateProgram = "UPDATE `tbl_course` SET `status` = '2' WHERE `sl_no` = '$programId'";
		    $resUpdateProgram = mysqli_query($conn, $queryUpdateProgram);
		    if ($resUpdateProgram) {
