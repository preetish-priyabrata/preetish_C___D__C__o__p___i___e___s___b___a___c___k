...
if (($_SESSION['userId']!="")) {
	...
	$batch_names=$_POST['batch_names'];
	  $student_query="SELECT * FROM `tbl_certificate_list` WHERE `batch_id`='$batch_names' and `course_id`='$course_names' ";
	...
	if($num=='0'){
		...
		<table id="example" class="display nowrap table-striped" cellspacing="0">
			...
			<tbody>
				<?php 
					$student_query1="SELECT DISTINCT `student_roll` FROM `tbl_certificate_list` WHERE `batch_id`='$batch_names' and `course_id`='$course_names'  ";
				$student_sql1=mysqli_query($conn,$student_query1);
				while ($res=mysqli_fetch_assoc($student_sql1)) {
					// print_r($res);
					$roll=$res['student_roll'];
					$student_query2="SELECT * FROM `tbl_certificate_list` WHERE `batch_id`='$batch_names' and `course_id`='$course_names' and `student_roll`='$roll'  ";
					$student_sql2=mysqli_query($conn,$student_query2);
					$student_query3="SELECT * FROM `tbl_certificate_list` WHERE `batch_id`='$batch_names' and `course_id`='$course_names' and `student_roll`='$roll'  ";
