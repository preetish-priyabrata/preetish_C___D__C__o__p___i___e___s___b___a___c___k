						  ...
						  <label class="control-label col-lg-4 text-right">Site Store Location</label>
						    ...
						    <div class="col-lg-8">
							   <select class="form-control" name="zonal_code" id="zonal_code" type="dropdown" required="">
	                    			...
	                    			<option value="<?php echo $rec_loc['zonal_code'];?>" <?php if($zonal_code==$rec_loc['zonal_code']){ echo "selected";}?>><?=$rec_loc['zonal_name'];?></option>
	             					 <?php }?>
	               			    </select>
						     </div>
						 </div>
						  ...
						  <label class="control-label col-lg-4 text-right">Machine Model No</label>
						  	<div class="col-lg-8">
								<select class="form-control" name="model_no" id="demo" type="dropdown" required="">
	                    			...
	                    			<option value="<?php echo $rec['model_id'];?>"  <?php if($model_no==$rec['model_id']){ echo "selected";}?> ><?=$rec['model_no'];?></option>
	             					 <?php }?>
	               			   </select>
						   	</div>
						 </div>
	        					...
	        					<tbody>
	        						<?php 
	        							...
	        							while ($result=mysqli_fetch_assoc($sql_exe)) {
	        					     	...
	        					     	<tr>
	        							<td><?=$x?></td>
	        							<td><input type="hidden" name="machine_unique_id[]" value="<?=$result['machine_unique_id']?>"><?=$result['machine_unique_id']?></td>
