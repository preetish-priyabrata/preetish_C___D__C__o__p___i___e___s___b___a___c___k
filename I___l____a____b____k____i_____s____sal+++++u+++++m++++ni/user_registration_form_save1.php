...
if(isset($_POST['submit'])){
    ...
    $search="SELECT * FROM `master_user_registration` WHERE `email`='$email' or `Mobile_no`='$mobile'";
	$search_exe=mysqli_query($conn,$search);

	$num=mysqli_num_rows($search_exe);
	...
	if(!empty($_FILES['img']['name'])){
  			...
  			if($sql_exe){
				echo 'Registration successfully';  
				 $msg->success('Registered Successfully '); 
				  $msg->success('Your Approval Details Shall We Sent Your Registered Email Id ');
	  ...
	  // elseif($password != $cn_password){
   //      $msg->error('passwords doesnot match');
   ...
   //  }
	...
	$msg->error('Insert Image Properly'); 
	header("location:user_login.php");
	...
	$msg->error('Your Email Id/Phone No Is Already Registered'); 
	header("location:user_login.php");
	//header('Location:http://alumni.ksom.ac.in');
    exit;
  }
