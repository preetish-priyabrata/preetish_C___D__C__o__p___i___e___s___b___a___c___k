  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      <?php
        ...
        if(!empty($village)){
           $get_detail="SELECT * FROM `care_master_hhi_month_year` WHERE `care_crp_id`='$employee_id' and`care_village_name`='$village' and `care_month`='$months' and `care_year`='$Year' and `care_form6`!=''";
                              ...
                              $Input_out5=1;
                               ...
                               $care_mt_status_form6=unserialize($sql_get_detail_fetch['care_mt_status_form6']);
                                  if(!empty($form6)){
                                    ...
                                    if($Input_out5!="2"){
                                     ?>
                                    <a class="btn btn-danger btn-xs" href="report_MT_go_form_go.php?form_type=<?=web_encryptIt('ilab')?>&form_id=<?=web_encryptIt('8')?>&care_hhi=<?=web_encryptIt($care_hhi_id_info)?>&target=<?=web_encryptIt($months)?>&year=<?=web_encryptIt($Year)?>&village=<?=web_encryptIt($village)?>&form_uses=2" >click to View <?=$x2?></a>
                                    <?php
                                  }else{
                                    ?>
                                     <a class="btn btn-danger btn-xs" href="report_MT_go_form_go.php?form_type=<?=web_encryptIt('ilab')?>&form_id=<?=web_encryptIt('8')?>&care_hhi=<?=web_encryptIt($care_hhi_id_info)?>&target=<?=web_encryptIt($months)?>&year=<?=web_encryptIt($Year)?>&village=<?=web_encryptIt($village)?>&form_uses=2" >>click to View <?=$x6?></del></a>
                                    <?php
                                  }
                                  }else{
                                    echo "N/A";
