                        ...
                        <td><?php
                              ...
                              $care_mt_status_form3=unserialize($sql_get_detail_fetch['care_mt_status_form3']);
                                  if(!empty($form3)){
                                    ...
                                    if($Input_out2!="2"){
                                      ?>
                                        <a class="btn btn-danger btn-xs" href="report_MT_go_form_go.php?form_type=<?=web_encryptIt('ilab')?>&form_id=<?=web_encryptIt('9')?>&care_hhi=<?=web_encryptIt($care_hhi_id_info)?>&target=<?=web_encryptIt($months)?>&year=<?=web_encryptIt($Year)?>&village=<?=web_encryptIt($village)?>&form_uses=2" >click to View <?=$x3?></a>
                                      <?php
                                    }else{
                                     ?>
                                        <a class="btn btn-danger btn-xs" href="report_MT_go_form_go.php?form_type=<?=web_encryptIt('ilab')?>&form_id=<?=web_encryptIt('9')?>&care_hhi=<?=web_encryptIt($care_hhi_id_info)?>&target=<?=web_encryptIt($months)?>&year=<?=web_encryptIt($Year)?>&village=<?=web_encryptIt($village)?>&form_uses=2" >click to View <?=$x3?></a>
                        ...
                        <td><?php
                               ...
                               $care_mt_status_form4=unserialize($sql_get_detail_fetch['care_mt_status_form4']);
                                  if(!empty($form4)){
                                    ...
                                    if($Input_out3!="2"){
                                     ?>
                                      <a class="btn btn-danger btn-xs" href="report_MT_go_form_go.php?form_type=<?=web_encryptIt('ilab')?>&form_id=<?=web_encryptIt('10')?>&care_hhi=<?=web_encryptIt($care_hhi_id_info)?>&target=<?=web_encryptIt($months)?>&year=<?=web_encryptIt($Year)?>&village=<?=web_encryptIt($village)?>&form_uses=2" >click to comment <?=$x4?></a>
                                    <?php
                                  }else{?>
                                    <a class="btn btn-danger btn-xs" href="report_MT_go_form_go.php?form_type=<?=web_encryptIt('ilab')?>&form_id=<?=web_encryptIt('10')?>&care_hhi=<?=web_encryptIt($care_hhi_id_info)?>&target=<?=web_encryptIt($months)?>&year=<?=web_encryptIt($Year)?>&village=<?=web_encryptIt($village)?>&form_uses=2" >click to View <?=$x4?> </a>
                                 <?php }
                                  }else{
                                    echo "N/A";
                                  }
                        ...
                        <td><?php
                                  ...
                                  if(!empty($form5)){
                                   for ($i=0; $i <count($form5) ; $i++) {
                                    ...
                                    if($Input_out4!="2"){
                                     ?>
                                        <a class="btn btn-danger btn-xs" href="report_MT_go_form_go.php?form_type=<?=web_encryptIt('ilab')?>&form_id=<?=web_encryptIt('11')?>&care_hhi=<?=web_encryptIt($care_hhi_id_info)?>&target=<?=web_encryptIt($months)?>&year=<?=web_encryptIt($Year)?>&village=<?=web_encryptIt($village)?>&form_uses=2" >click to comment <?=$x5?></a>
                                    <?php
                                  }else{
                                    ?>
                                     <a class="btn btn-danger btn-xs" href="report_MT_go_form_go.php?form_type=<?=web_encryptIt('ilab')?>&form_id=<?=web_encryptIt('11')?>&care_hhi=<?=web_encryptIt($care_hhi_id_info)?>&target=<?=web_encryptIt($months)?>&year=<?=web_encryptIt($Year)?>&village=<?=web_encryptIt($village)?>&form_uses=2" >click to View <?=$x5?></a>
                                  <?php
                                  }
                                  }else{
                                    echo "N/A";
