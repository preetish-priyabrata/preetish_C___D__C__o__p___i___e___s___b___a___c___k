...
if($_SESSION['admin_zonal']){
	...
	while ($result=mysqli_fetch_assoc($sql_Requsition_exe)) {
		...
		$damage_loss=$results['damage_loss'];
		$new_damage_loss=$damage_loss+$fsindi_secondary_code;

		 $update="UPDATE `lt_master_zonal_stock_info` SET `damage_loss`='$new_damage_loss',`damage_loss_status`='1' WHERE`zonal_slno`='$zonal_slno'";
	...
	if($sql_update_challan){
		$msg->success('Successfully Receive Of damage Challan');
