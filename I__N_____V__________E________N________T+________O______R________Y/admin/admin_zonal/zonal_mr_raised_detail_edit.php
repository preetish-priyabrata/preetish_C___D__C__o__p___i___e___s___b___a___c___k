					        					...
					        					<tbody>
					        						<?php 
										               ...
										               <td><?=strtoupper($fetch_item['zmrd_category_name'])?></td>
										                ...
										                <td><input type="number" name="quantity_req[]" placeholder="Please Enter Quantity" value="<?=strtoupper($fetch_item['zmrd_request_qnt'])?>" > </td>

										                <td><input type="text" name="remarks_detail[]" placeholder="Please Enter Remark" value="<?=strtoupper($fetch_item['remark_id'])?>" ></td>

										                <td><a class="btn btn-success" href="zonal_mr_raised_remove.php?slno=<?=$slno_en?>&list=<?=$list_en?>&zmrd_slno=<?=web_encryptIt($zmrd_slno)?>&re=2">Remove</a></td>
						            				</tr>
					        						   <?php
					        							}
					        							?>
