...
if($_SESSION['admin']){
  ...
  //   $program = decryptIt($program);
		// Array ( [slno] => 1 [unq_id] => hq1 [status] => 1 ) 
		$u_slno=decryptIt_web(str_replace(" ", "+", $_REQUEST['u_slno']));
		$unq_id=decryptIt_web(str_replace(" ", "+", $_REQUEST['unq_id']));
		$u_status=decryptIt_web(str_replace(" ", "+", $_REQUEST['u_status']));
		if($u_status!="0"){
			if($u_status=="2"){
