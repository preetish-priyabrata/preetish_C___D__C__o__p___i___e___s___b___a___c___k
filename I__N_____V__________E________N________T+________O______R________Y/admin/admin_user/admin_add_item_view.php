                              ...
                              <table id="example" class="display nowrap" width="100%" cellspacing="0">
	        					...
	        					<tbody>
	        						<?php 
	        							...
	        							<td><?=strtoupper($x)?></td>
	        							<td><?=strtoupper($result['hsn_code'])?></td>
	        							<td><?=strtoupper($result['item_primary_code'])?></td>
	        						    <td><?=strtoupper($result['item_second_code'])?></td>
	        						    <td><?=strtoupper($result['item_name'])?></td>
	        						    <td><?=strtoupper($result['item_category_id'])?></td>
	        						    <td><?=strtoupper($result['brand_name'])?></td>
	        						   
	        						    <td><?=strtoupper($result['manufacture_name'])?></td>
                                       <td>

										<?php if($result['status']=="2"){ ?>
	        							<a href="admin_add_item_update_status.php?slno=<?=$result['slno']?>&status=<?=$result['status']?>">In-Active</a>
	        							<?php }else{  ?>

	        							<a href="admin_add_item_update_status.php?slno=<?=$result['slno']?>&status=<?=$result['status']?>">Active</a>
	        						<?php }?>
