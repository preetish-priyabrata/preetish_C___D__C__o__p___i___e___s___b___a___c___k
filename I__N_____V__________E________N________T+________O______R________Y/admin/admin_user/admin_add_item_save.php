	            ...
	            while($rec = mysqli_fetch_assoc($exe_vieew)){
	            	...
	            	$check_model="SELECT * FROM `lt_master_model_item_detail` WHERE `i_item_slno`='$last_id' and `i_model_id`='$model_no'";
			   		...
			   		if($num_check=="0"){ // IF IT DOES NOT FIND ANY ROW AFFECTED THEN IT WILL WORK FOR STORING INFORMATION IN MODEL TABLE 
			   			$query_insert_model="INSERT INTO `lt_master_model_item_detail`(`i_slno`, `i_item_slno`, `i_model_id`, `item_name`, `item_primary_code`, `item_second_code`, `item_category_id`, `status`, `date`, `time`,`it_de_unit`,`item_hsn_code`) VALUES (Null,'$last_id','$model_no','$item_name','$item_primary_code','$item_second_code','$item_category_id','1','$date','$time','$name_unit','$hsn_code')";
