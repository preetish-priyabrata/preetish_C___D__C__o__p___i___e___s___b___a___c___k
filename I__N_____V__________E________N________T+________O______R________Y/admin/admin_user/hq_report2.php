							...
							<div class="form-group">
							    ...
							    <div class="col-lg-8">
							    	<select name="location" >
							    		<option value="" <?php if($location1==$location){echo "selected";}?>>-Select Site Name-</option>
							    		<?php 
			        		...
			        		<table id="example77" class="display nowrap" width="100%" cellspacing="0">
				        		...
				        		<tbody>
				        			...
				        			if($location!=""){
				        				 ...
				        				 $GET_INFO="SELECT * FROM `lt_master_zonal_material_requsition` WHERE  `date`BETWEEN '$date_one' AND '$date_two'";
				        			}
		        				...
		        				while ($res=mysqli_fetch_assoc($sql_exe)) {
		        					...
		        					<tr>
		        						<td><?=$x?></td>
		        						<td><?PHP 
		        						$zmr_site_from_location_id=$res['zmr_site_from_location_id'];
		        						$query_LOACTION="SELECT * FROM `lt_master_zonal_place` WHERE   `status`='1' AND `zonal_code`='$zmr_site_from_location_id'";
											$sql_exe_LOC=mysqli_query($conn,$query_LOACTION);
											$res_loaction=mysqli_fetch_assoc($sql_exe_LOC);
											echo strtoupper($res_loaction['zonal_name']);

											$hq_code=$res_loaction['hq_code'];
											// `hq_code`='$hq_id'
											// 
											$query_sql="SELECT * FROM `lt_master_hq_place` WHERE `hq_code`='$hq_code'";											
											$sql_exe_L=mysqli_query($conn,$query_sql);

	        									while ($result=mysqli_fetch_assoc($sql_exe_L)) {
	        										echo " [".strtoupper($result['hq_name'])."]";
	        									}

		        						?></td>
									    <td><?=strtoupper($res['zmr_unqiue_mr_id'])?></td>
			 						    <td><?=$res['date']?></td>
			 						    <td><?=$res['time']?></td>
			 						    <td>
				        					<div class="btn-group">
