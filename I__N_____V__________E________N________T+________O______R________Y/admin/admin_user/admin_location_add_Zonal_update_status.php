...
if($_SESSION['admin']){
	...
	require 'config.php';
 	$msg = new \Preetish\FlashMessages\FlashMessages();
		...
		if($status=="1"){
			$query="UPDATE `lt_master_zonal_place` SET `status`='2' where `slno`='$slno' AND`zonal_code`='$unq_id'";
		}else{
			$query="UPDATE `lt_master_zonal_place` SET `status`='1' where `slno`='$slno' AND`zonal_code`='$unq_id'";
		}
