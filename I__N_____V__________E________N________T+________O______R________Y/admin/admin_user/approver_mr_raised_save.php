...
if($_SESSION['admin']){
	...
	if($form_type=="approver_edited"){
		...
		for ($i=0; $i < count($slno_item) ; $i++) { 
			...
			$UPDATE="UPDATE `lt_master_zonal_material_requsition_details` SET `zmrd_approved_qnt`='$quantity_req_single',`zmrd_approve_status`='1',`zmrd_approver_date`='$date_info',`zmrd_approver_time`='$Time_info' WHERE`zmrd_slno`='$slno_item_single'";
