...
$title="Field User Edit Material Requsition form ";
...
.form-control[disabled], .form-control[readonly] {
	...
	<div class="header">
		<div class="header-content">
			<div class="page-title"><i class="icon-display4"></i> Edit Material Requsition Form </div>
			<ul class="breadcrumb">
				<li><a href="zonal_dashboard.php"></a></li>
				<li class="">Dashboard</li>
				<li class="">Requisition Management </li>
				<li class="">Your Site M.R Request</li>
				<li class="active">Edit Material Requisition Form </li>
			</ul>
		</div>
	</div>
	<!-- /Page Header-->
				 ...
				 <div class="panel-title">Requisition Form</div>
				     ...
				     <div class="panel-body">
					   	<form name="myFunction" class="form-horizontal" action="field_mr_raised_machine_save.php" method="POST">
					   		<input type="hidden" name="form_type" value="<?=web_encryptIt('field_mr3')?>">
						   	<div class="col-lg-6">
					        					...
					        					<tbody>
					        						<?php 
					        							...
					        							while ($fetch_item=mysqli_fetch_assoc($sql_item_exe)) {
					        								...
					        								$get_detais="SELECT * FROM `lt_master_field_stock_info` WHERE `field_secondary_code`='$fetch_item[fmrd_second_code]' and `field_location_id`='$field_place'";
					        								$sql_get_detais_exe=mysqli_query($conn,$get_detais);
					        								$fetch_get_detais=mysqli_fetch_assoc($sql_get_detais_exe);
					        								if(mysqli_num_rows($sql_get_detais_exe)==0){
					        									$field_qnty='0';
					        								}else{
					        									$field_qnty=$fetch_get_detais['field_qnty'];
					        								}
					        								$x++;
					        								?>
					        								<tr>
						                 				<td><?=$x?></td>
				 									    <!-- <td>Primary Code</td> -->
										                <td><?=strtoupper($fetch_item['fmrd_second_code'])?></td>	
										               ...
										               <td><?=strtoupper($fetch_item['fmrd_category_name'])?></td>
										                <td><?=$field_qnty?>
										                <input type="hidden" name="quantity_aval[]" value="<?=$field_qnty?>" required=""> <?=strtoupper($fetch_item['fmrd_units_required'])?></td>
										                <td>
										                
										                <input type="number" name="quantity_req[]" placeholder="Please Enter Quantity" min="0" value="<?=strtoupper($fetch_item['fmrd_request_qnt'])?>" required=""> 
										                <?=strtoupper($fetch_item['fmrd_units_required'])?>
										                </td>

										                <td><input type="text" name="remarks_detail[]" value="<?=strtoupper($fetch_item['remark_id'])?>" placeholder="Please Enter Remark" required></td>

										                <td><a class="btn btn-success" href="field_mr_raised_remove.php?slno=<?=$slno_en?>&list=<?=$list_en?>&fmrd_slno=<?=web_encryptIt($fetch_item[fmrd_slno])?>&re=2&machine_no=<?=$machine_no?>">Remove</a></td>
						            				</tr>
					        						  <?php
					        							}
					        							?>
