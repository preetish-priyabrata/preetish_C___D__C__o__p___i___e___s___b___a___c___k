...
.panel-body p {
	...
	<div class="header">
		<div class="header-content">
			<div class="page-title"><i class="icon-display4"></i>Report On Site Return</div>
			<ul class="breadcrumb">
				<li><a href="zonal_dashboard.php"></a></li>
				<li class="">Dashboards</li>
				<li class="">Reporting</li>
				<li class="active">Report On Site Return</li>
			</ul>
		</div>
	</div>
	<!-- /Page Header-->
		...
		<?php $msg->display(); ?>
			 ...
			 <div class="panel panel-default">
			   <div class="panel-heading">
				 <div class="panel-title">Report On Site Return</div>
				   </div>
				     ...
				     <div class="panel-body">
					   	<div class="table-responsive">
			        		<table id="example77" class="display nowrap" width="100%" cellspacing="0">
				        		...
				        		<tbody>
				        			<?php 
				        			// SELECT * FROM `lt_master_zonal_internal_damage_info` WHERE `zsindi_return_location_id` and `zsindi_date`
				        			$x=0;
		        				 $GET_INFO="SELECT * FROM `lt_master_zonal_internal_damage_info` WHERE `zsindi_return_location_id`='$location' AND `zsindi_date`BETWEEN '$date_one' AND '$date_two'";
		        				...
		        				while ($res=mysqli_fetch_assoc($sql_exe)) {
		        					...
		        					<tr>
		        						<td><?=$x?></td>
									    <td><?=strtoupper($res['zsindi_challan_no'])?></td>
			 						    <td><?=strtoupper($res['zsindi_component_name'])?></td>
			 						    <td><?=strtoupper($res['zsindi_hsn_code'])?></td>
			 						    <td><?=strtoupper($res['zsindi_primary_code'])?></td>
			 						    <td><?=strtoupper($res['zsindi_secondary_code'])?></td>
			 						    <td><?=strtoupper($res['zsindi_qnt'])?> <?=strtoupper($res['zsindi_unit'])?></td>
			 						    <td>INR <?=round($res['rate_price_unit'],2)?></td>
									    <td><?=strtoupper($res['zsindi_date'])?></td>
		        					</tr>
		        					<?php }?>
		        				</tbody>
				        	</table>
