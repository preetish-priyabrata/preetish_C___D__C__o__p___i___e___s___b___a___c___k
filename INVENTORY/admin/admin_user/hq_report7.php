				     ...
				     <div class="panel-body">
					   	<form name="myFunction" class="form-horizontal" action="" method="get" >
							...
							<!-- <div class="form-group">
							    ...
							    <div class="col-lg-8">
							    	<select name="location" onchange="detail_location()" id="location" >
							    		...
							    		<?php 
							    			$query="SELECT Distinct(`t_unique_id_machine`) FROM `lt_master_machine__transfer_information` ";
												$sql_exe=mysqli_query($conn,$query);				
													while ($res=mysqli_fetch_assoc($sql_exe)) {
													?>
														<option value="<?=$res['t_unique_id_machine']?>" <?php if($res['t_unique_id_machine']==$machine){echo "Selected";}?>><?=strtoupper($res['t_unique_id_machine'])?></option>
														<?php
													}
												?>
							    	</select>
					   	...
					   	<div class="table-responsive">
			        		<table id="" class="display nowrap table" width="100%" cellspacing="0">
				        		...
				        		<tbody>
		        					...
		        					if($machine==""){
		        						$GET_INFO="SELECT * FROM `lt_master_machine_uses` WHERE  `date_enter` BETWEEN '$date_one' AND '$date_two'";
		        					}else{
		        						$GET_INFO="SELECT * FROM `lt_master_machine_uses` WHERE  `Machine`='$machine' AND `date_enter` BETWEEN '$date_one' AND '$date_two'";
		        					}
		        						...
		        						<td><?=$res['Machine']?></td>									    
			 						    ...
			 						    <td><?php 
				 						    if($res['nos_attach_files']==0){
				 						    	...
				 						    	$slno=$res['slno'];
				 						    	$get_file="SELECT * FROM `lt_master_machine_efficentent_attach` WHERE `slno_id`='$slno'";
				 						    	$sql_exe_get_file=mysqli_query($conn,$get_file);
				 						    	while ($res_get_file=mysqli_fetch_assoc($sql_exe_get_file)) {
				 						    	$targetDir="../uploads/machine_performance/".$res_get_file['attach_name'];
				 						    	$filename=$res_get_file['attach_name'];
					 						    	if (file_exists($targetDir)) {
													   echo "<a target='_blank' href='".$targetDir."'>Click View</a><br>";
													} else {
													    echo "The Attachment not Found";
													}
												}
				 						    }
			 						    ?></td>
		        				...
		        				<tfoot>
					            	<tr>
					            		<th>Total</th>	
					            		<td>:		 						   
			 						    <th><?=strtoupper($w_h)?></th>
			 						    <th><?=strtoupper($i_h)?></th>
									    <th><?=strtoupper($m_t)?></th>
									    <th><?=strtoupper($b_d_mcl)?></th>
			 						    <th><?=strtoupper($b_d_lnt)?></th>
									    <th><?=strtoupper($avl_hrs)?></th>
									    <th><?=strtoupper($shift_hrs)?> </th>
			 						    <th><?=strtoupper($picks)?></th>
									    <th><?=strtoupper($diesel)?></th>
									    <th><?=strtoupper($production)?></th>
			 						    <th></th>
			 						    <th></th>
									</tr>
				        		</tfoot>
