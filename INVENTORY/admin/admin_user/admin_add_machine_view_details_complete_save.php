...
$num=mysqli_num_rows($sql_check);
	if($num==1){
		...
		if(is_array($_FILES)) {
			...
			$fetch=mysqli_fetch_assoc($sql_check);
			$m_count=$fetch['m_count'];
			$new_m_count=$m_count+1;
			$update="UPDATE `lt_master_machine__transfer_information` SET `t_status`='2' WHERE `slno`='$slno'";
			mysqli_query($conn,$update);
			$update_machine="UPDATE `lt_master_machine_detail` SET `m_count`='$new_m_count',`assign_status`='0' WHERE`machine_unique_id`='$unique_id_machine'";
			mysqli_query($conn,$update_machine);
			$msg->success('Machine Successfull Task is completed');
