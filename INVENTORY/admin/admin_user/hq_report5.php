							    ...
							    <div class="col-lg-8">
							    	<select name="location" required>
										...
										while ($res=mysqli_fetch_assoc($sql_exe)) {
												?>
													<option value="<?=$res['zonal_code']?>" <?php if($res['zonal_code']==$location){echo "selected";}?>><?=strtoupper($res['zonal_name'])?>
				     ...
				     <div class="panel-body">
					   	<div class="table-responsive">
			        		<table id="example77" class="display nowrap" width="100%" cellspacing="0">
				        		...
				        		<tbody>
				        			<?php 
				        			// `zmrd_slno`, `zmrd_slno_id`, `zmr_unqiue_mr_id_item`, `zmrd_site_location_id`, `zmrd_primary_code`, `zmrd_hsn_code`, `zmrd_second_code`, `zmrd_name_item`, `zmrd_category_name`, `zmrd_category_id`, `zmrd_units_required`, `zmrd_machine_no`, `zmrd_avaliable_qnt`, `zmrd_request_qnt`, `zmrd_approved_qnt`, `zmrd_issue_qnt`, `zmr_price_rate`, `zmr_total_price`, `zmrd_date_entry`, `zmrd_time_entry`, `zmrd_approver_date`, `zmrd_approver_time`, `zmrd_issue_date`, `zmrd_issue_time`, `zmrd_issue_status`, `transfer_status`, `zmrd_approve_status`, `no_days_approve`, `no_days_issued`, `remark_id`, `receive_qnty`, `receive_qnty_status`, `damage_loss`, `tranisit_loss`, `tranist_status`, `damage_loss_status`
				        			$x=0;
		        				...
		        				while ($res=mysqli_fetch_assoc($sql_exe)) {
		        					...
		        					<tr>
		        						<td><?=$x?></td>
									    <td><?=strtoupper($res['zmr_unqiue_mr_id_item'])?></td>
			 						    <td><?=strtoupper($res['zmrd_name_item'])?></td>
			 						    <td><?=strtoupper($res['zmrd_hsn_code'])?></td>
			 						    <td><?=strtoupper($res['zmrd_primary_code'])?></td>
			 						    <td><?=strtoupper($res['zmrd_second_code'])?></td>
			 						    <td><?=strtoupper($res['zmrd_category_name'])?></td>
									    <td><?=strtoupper($res['zmrd_request_qnt'])?> <?=strtoupper($res['zmrd_units_required'])?></td>
									    
									    <td><?=strtoupper($res['zmrd_date_entry'])?></td>
		        					</tr>
		        					<?php }?>
		        				</tbody>
				        	</table>
