...
if($_SESSION['admin_zonal']){
	...
	$new_price=$total_price-($price_charge_unit*$damage_qnty);

	$damage_qnty_price=$_POST['totalprice'];

	 $insert="INSERT INTO `lt_master_zonal_stock_transfer_info`(`zqt_slno`, `zqt_primary_code`, `zqt_secondary_code`, `zqt_component_name`, `zqt_component_id`, `zqt_category_name`, `zqt_category_id`, `zqt_unit`, `zqt_qnty`, `zqt_date`, `zqt_time`, `zqt_location_id`, `zqt_transfer_location`, `zqt_transfer_type`, `zqt_remark`, `zqt_opening_balance`, `zqt_closing_balance`, `damage_loss`, `damage_loss_status`,`price_issue_rate`,`total_price_item`,`total_receive_price`) VALUES (Null,'$zonal_primary_code','$zonal_secondary_code','$zonal_component_name','$zonal_component_id','$zonal_category_name','$zonal_category_id','$zonal_unit','$damage_qnty','$date','$time','$zonal_location_id','$zonal_location_id','5','damage was found please in stock','$zonal_qnty','$closed','$damage_qnty','1','$price_charge_unit','$damage_qnty_price','$damage_qnty_price')";
	$sql_insert=mysqli_query($conn,$insert);
	 $update="UPDATE `lt_master_zonal_stock_info` SET `damage_loss`='$damage_loss_s',`zonal_qnty`='$closed',`damage_loss_status`='1',`total_price`='$new_price' WHERE  `zonal_slno`='$zonal_slno'";
