...
$list_en= $_GET['list'];
// unset($_GET);
$check="SELECT * FROM `lt_master_zonal_material_requsition` where `zmr_slno`='$slno' and `zmr_unqiue_mr_id`='$list' and `status`='0'";
				 ...
				 <div class="panel-title">Requsition Form</div>
				     ...
				     <div class="panel-body">
					   	<form name="myFunction" class="form-horizontal" action="zonal_mr_raised_save.php" method="POST">
					   		<input type="hidden" name="form_type" value="<?=web_encryptIt('site_mr2')?>">
						   	<div class="col-lg-6">
				                           ...
				                           <table class="display nowrap table" width="100%" cellspacing="0">
					        					...
					        					<tbody>
					        						<?php 
										                ...
										                <td><?php $zmrd_second_code=$fetch_item['zmrd_second_code']?><?=strtoupper($fetch_item['zmrd_second_code'])?></td>	
										               ...
										               <td><?=strtoupper($fetch_item['zmrd_category_name'])?></td>
										              	...
										              	?>
										                <td><?=$zonal_qnty?>  
										                <input type="hidden" value="<?=$zonal_qnty?>" name="quantity_aval[]" value="0" required=""> <?=strtoupper($fetch_item['zmrd_units_required'])?></td>

										                <td><input type="number" min="0" name="quantity_req[]"  placeholder="Please Enter Quantity" > <?=strtoupper($fetch_item['zmrd_units_required'])?></td>

										                <td><input type="text" name="remarks_detail[]" placeholder="Please Enter Remark"></td>
										                
										                <td><a class="btn btn-success" href="zonal_mr_raised_remove.php?slno=<?=$slno_en?>&list=<?=$list_en?>&zmrd_slno=<?=web_encryptIt($zmrd_slno)?>&re=1">Remove</a></td>
						            				</tr>
					        						   <?php
					        							}
					        							?>
