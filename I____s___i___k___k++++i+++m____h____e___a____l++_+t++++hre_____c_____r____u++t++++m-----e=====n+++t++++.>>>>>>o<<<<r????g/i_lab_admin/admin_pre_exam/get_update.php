...
switch ($pay_id) {
	...
	case '2':
		...
		break;
	case '3':
		$query_sql="SELECT * FROM `ilab_login` WHERE `complete_status`='1'";
        $sql_exe=mysqli_query($conn,$query_sql);
        $fetch=mysqli_num_rows($sql_exe);
        // print_r($fetch);
        echo $fetch;
		break;
	case '4':
		$query_sql="SELECT * FROM `ilab_login` WHERE `complete_status`='0' and `basic_status`='1'";
        $sql_exe=mysqli_query($conn,$query_sql);
        $fetch=mysqli_num_rows($sql_exe);
        // print_r($fetch);
        echo $fetch;
	...
	case '5':
		...
		break;
	case '6':
		$query_sql="SELECT * FROM `ilab_login` WHERE `complete_status`='0' and `basic_status`='0'";
        $sql_exe=mysqli_query($conn,$query_sql);
        $fetch=mysqli_num_rows($sql_exe);
        // print_r($fetch);
        echo $fetch;
		break;
	case '7':
		$query_sql="SELECT * FROM `ilab_login` ";
        $sql_exe=mysqli_query($conn,$query_sql);
        $fetch=mysqli_num_rows($sql_exe);
        // print_r($fetch);
        echo $fetch;
