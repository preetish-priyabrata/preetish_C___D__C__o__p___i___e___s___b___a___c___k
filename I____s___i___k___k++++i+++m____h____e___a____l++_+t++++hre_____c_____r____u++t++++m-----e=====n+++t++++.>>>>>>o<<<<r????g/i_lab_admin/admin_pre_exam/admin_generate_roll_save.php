...
if($_SESSION['admin_preexam']){
 ...
 // exit;	// Array ( [exam] => 1 )
	...
	// exam check bracket
	if($num_check=='1'){
		// ilab_group_candidate_info_paid_paid

		// echo $check_candidate="SELECT * FROM `ilab_group_candidate_info_paid` WHERE `group_id_slno`='$exam' and `paid_status`='1' and `roll_status`='0'";
		 $check_candidate="SELECT * FROM `ilab_group_candidate_info_paid` left JOIN `ilab_payment_info_PAID` on ilab_payment_info_PAID.paid_info_slno=ilab_group_candidate_info_paid.paid_id_slno and ilab_payment_info_PAID.mobile_no=ilab_group_candidate_info_paid.candidate_mobile WHERE ilab_group_candidate_info_paid.paid_status='1' and ilab_group_candidate_info_paid.group_id_slno='$exam'";
		// SELECT * FROM `ilab_group_candidate_info_paid` left JOIN `ilab_payment_info_PAID` on ilab_payment_info_PAID.paid_info_slno=ilab_group_candidate_info_paid.paid_id_slno and ilab_payment_info_PAID.mobile_no=ilab_group_candidate_info_paid.candidate_mobile WHERE ilab_group_candidate_info_paid.paid_status='1' and ilab_group_candidate_info_paid.group_id_slno='1' 
		// exit();
		$sql_candidate=mysqli_query($conn, $check_candidate);
		$total=$num_check_candidate=mysqli_num_rows($sql_candidate);
		...
		if($num_check_candidate!=0){
			...
			while($fetch_user=mysqli_fetch_assoc($sql_candidate)){
				...
				//  $get_post_inform="SELECT * FROM `ilab_payment_info_PAID` WHERE `slno_group_pay`='$paid_id_slno' and `group_id_slno`='$exam'";
				...
				$post_ids_info=json_decode($fetch_user['post_ids_info']);
				 $get_post_ids_info=$post_ids_info[0];
					
					$total_count_id=$fetch_user['total_count'];
				$total_count=sprintf("%02d", $total_count_id);
				$candidate_application=$fetch_user['application_no'];
				unset($fix1);
				unset($x);
				unset($insert);
				unset($update_pay_roll);
				$file = fopen("ilab_payment_info_PAID.txt", "a+");
				fwrite($file, "---". $i."->" . $paid_id_slno . "-> ".$get_post_ids_info."  ---  ");
				...
				// unset($update_pay_roll);
				 $update_pay_roll ="UPDATE `ilab_group_candidate_info_paid` SET `roll_status`='1' WHERE `slno_ca_group`='$slno_ca_group';";
				
				 $update_pay_roll1 ="UPDATE `ilab_payment_info_PAID` SET `gen_roll_status`='1' WHERE`slno_group_pay`='$slno_group_pay';";
				 
				$file = fopen("ilab_pre_exam_roll_no.txt", "a+");
				fwrite($file, "---" . $insert . "---");
				...
				fwrite($file, "--- paid group -> " . $slno_ca_group . "--- Paid id Info -> ".$slno_group_pay."---");
				echo '<script language="javascript">
				    document.getElementById("progress").innerHTML="<div style=\"width:'.$percent.';background-image:url(pbar-ani.gif);\">&nbsp;</div>";
				    document.getElementById("information").innerHTML="'.$i.' to '.$total.' row(s) processed. Paid id Info' .$slno_group_pay.' paid group '.$slno_ca_group.' Roll no '.$roll_no.'";
				    </script>';
