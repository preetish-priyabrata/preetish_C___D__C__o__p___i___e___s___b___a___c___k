                              ...
                              <table id="example" class="display nowrap" width="100%" cellspacing="0">
	        					...
	        					<tbody>
	        						<?php 
	        							...
	        							$x=0;
	        							$query_sql="SELECT * FROM `lt_master_user_system` WHERE `u_status`!='0'";
	        							...
	        							<td><?=strtoupper($result['user_designation'])?></td>
	        						    <td>
		        						    <?php 
	        						    ...
	        						    <td><?=strtoupper($result['user_email'])?></td>
	        						    <td><?=strtoupper($result['user_mobile'])?></td> 

	        						    <td><?php if($result['u_status']=="2"){ ?>
	        								<a href="admin_add_Users_update_status.php?u_slno=<?=encryptIt_web($result['u_slno'])?>&unq_id=<?=encryptIt_web($result['user_id'])?>&u_status=<?=encryptIt_web($result['u_status'])?>">In-Active</a>
	        							<?php }else{  ?>
	        								<a href="admin_add_Users_update_status.php?u_slno=<?=encryptIt_web($result['u_slno'])?>&unq_id=<?=encryptIt_web($result['user_id'])?>&u_status=<?=encryptIt_web($result['u_status'])?>">Active</a>
	        							<?php }?></td> 						    
	        							 ...
	        							 <div class="btn-group">
			                                    ...
			                                    <ul class="dropdown-menu">
			                                        <li><a href="admin_add_Users_view_details.php?u_slno=<?php echo $result['u_slno'] ;?>">View Detail</a></li>
